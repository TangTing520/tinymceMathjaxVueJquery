<template>
  <!-- <div class="Mathematical" :class="visible?'animation-scale':''"> -->
  <div class="Mathematical" :class="visible?'animation-scale':''">
    <div class="sybol">

      <h3 class="title">
        <p>数学符号</p>
        <p class="close" @click="close()">X</p>
      </h3>
      <form id="form1" runat="server">
        <div class="MathToolbar">
          <div class="Popover"></div>
          <span hidden>
            <svg hidden xmlns="http://www.w3.org/2000/svg">
              <symbol id="aleph_1603530409" viewBox="0 -791.3 611.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 613Q55 643 61 663T74 688T85 694Q94 694 94 681Q98 632 134 588L412 285Q416 311 430 397T447 509V519L438 526Q407 554 398 571T388 617T394 664T407 688T418 694Q425 694 427 684Q429 675 454 635T488 586Q490 584 496 579T513 563T537 540Q555 516 555 487Q555 460 549 441T537 416T528 409Q519 409 517 415T513 435T503 463Q492 481 490 481Q454 264 454 246Q454 237 479 212T529 152T555 79Q555 32 538 9Q531 1 524 1Q516 1 516 13Q512 62 476 106Q468 115 337 258T195 412L193 406Q191 401 189 394T183 377T176 352T171 322T167 284T165 240Q165 224 166 220Q171 199 211 152T252 70Q252 45 235 29T203 8T175 1Q170 0 115 0H79Q60 0 58 3T55 20Q55 31 58 34Q60 37 76 37Q112 39 126 46T140 70Q140 96 112 148T83 236Q83 281 102 334T140 419T159 452Q55 556 55 613Z"
                  />
                </g>
              </symbol>
              <symbol id="alpha_1318469421" viewBox="0 -576.1 640.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M34 156Q34 270 120 356T309 442Q379 442 421 402T478 304Q484 275 485 237V208Q534 282 560 374Q564 388 566 390T582 393Q603 393 603 385Q603 376 594 346T558 261T497 161L486 147L487 123Q489 67 495 47T514 26Q528 28 540 37T557 60Q559 67 562 68T577 70Q597 70 597 62Q597 56 591 43Q579 19 556 5T512 -10H505Q438 -10 414 62L411 69L400 61Q390 53 370 41T325 18T267 -2T203 -11Q124 -11 79 39T34 156ZM208 26Q257 26 306 47T379 90L403 112Q401 255 396 290Q382 405 304 405Q235 405 183 332Q156 292 139 224T121 120Q121 71 146 49T208 26Z"
                  />
                </g>
              </symbol>
              <symbol id="angle_1166132696" viewBox="0 -791.3 722.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M71 0L68 2Q65 3 63 5T58 11T55 20Q55 22 57 28Q67 43 346 361Q397 420 474 508Q595 648 616 671T647 694T661 688T666 674Q666 668 663 663Q662 662 627 622T524 503T390 350L120 41L386 40H653Q666 30 666 20Q666 8 651 0H71Z"
                  />
                </g>
              </symbol>
              <symbol id="approx_3715476797" viewBox="0 -576.1 778.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 319Q55 360 72 393T114 444T163 472T205 482Q207 482 213 482T223 483Q262 483 296 468T393 413L443 381Q502 346 553 346Q609 346 649 375T694 454Q694 465 698 474T708 483Q722 483 722 452Q722 386 675 338T555 289Q514 289 468 310T388 357T308 404T224 426Q164 426 125 393T83 318Q81 289 69 289Q55 289 55 319ZM55 85Q55 126 72 159T114 210T163 238T205 248Q207 248 213 248T223 249Q262 249 296 234T393 179L443 147Q502 112 553 112Q609 112 649 141T694 220Q694 249 708 249T722 217Q722 153 675 104T555 55Q514 55 468 76T388 123T308 170T224 192Q164 192 125 159T83 84Q80 55 69 55Q55 55 55 85Z"
                  />
                </g>
              </symbol>
              <symbol id="ast_2087933023" viewBox="0 -576.1 500.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M229 286Q216 420 216 436Q216 454 240 464Q241 464 245 464T251 465Q263 464 273 456T283 436Q283 419 277 356T270 286L328 328Q384 369 389 372T399 375Q412 375 423 365T435 338Q435 325 425 315Q420 312 357 282T289 250L355 219L425 184Q434 175 434 161Q434 146 425 136T401 125Q393 125 383 131T328 171L270 213Q283 79 283 63Q283 53 276 44T250 35Q231 35 224 44T216 63Q216 80 222 143T229 213L171 171Q115 130 110 127Q106 124 100 124Q87 124 76 134T64 161Q64 166 64 169T67 175T72 181T81 188T94 195T113 204T138 215T170 230T210 250L74 315Q65 324 65 338Q65 353 74 363T98 374Q106 374 116 368T171 328L229 286Z"
                  />
                </g>
              </symbol>
              <symbol id="asymp_1903012079" viewBox="0 -576.1 778.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 464Q55 471 60 477T74 484Q80 484 108 464T172 420T268 376T389 356Q436 356 483 368T566 399T630 436T675 467T695 482Q701 484 703 484Q711 484 716 478T722 464Q722 454 707 442Q550 316 389 316Q338 316 286 329T195 362T124 402T76 437T57 456Q55 462 55 464ZM57 45Q66 58 109 88T230 151T381 183Q438 183 494 168T587 135T658 94T703 61T720 45Q722 39 722 36Q722 28 717 22T703 16Q697 16 669 36T606 80T510 124T389 144Q341 144 294 132T211 101T147 64T102 33T82 18Q76 16 74 16Q66 16 61 22T55 36Q55 39 57 45Z"
                  />
                </g>
              </symbol>
              <symbol id="barx_3677001718" viewBox="0 -719.6 572.5 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                  />
                  <g transform="translate(63,7)">
                    <path stroke-width="1" d="M69 544V590H430V544H69Z" />
                  </g>
                </g>
              </symbol>
              <symbol id="beta_170234155" viewBox="0 -791.3 573.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M29 -194Q23 -188 23 -186Q23 -183 102 134T186 465Q208 533 243 584T309 658Q365 705 429 705H431Q493 705 533 667T573 570Q573 465 469 396L482 383Q533 332 533 252Q533 139 448 65T257 -10Q227 -10 203 -2T165 17T143 40T131 59T126 65L62 -188Q60 -194 42 -194H29ZM353 431Q392 431 427 419L432 422Q436 426 439 429T449 439T461 453T472 471T484 495T493 524T501 560Q503 569 503 593Q503 611 502 616Q487 667 426 667Q384 667 347 643T286 582T247 514T224 455Q219 439 186 308T152 168Q151 163 151 147Q151 99 173 68Q204 26 260 26Q302 26 349 51T425 137Q441 171 449 214T457 279Q457 337 422 372Q380 358 347 358H337Q258 358 258 389Q258 396 261 403Q275 431 353 431Z"
                  />
                </g>
              </symbol>
              <symbol id="bigcap_ab_1527652773" viewBox="0 -1867.7 1111.5 3161.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <g transform="translate(0,1)">
                    <path
                      stroke-width="1"
                      d="M57 516Q68 602 104 675T190 797T301 882T423 933T542 949Q594 949 606 948Q780 928 901 815T1048 545Q1053 516 1053 475T1055 49Q1055 -406 1054 -410Q1051 -427 1037 -438T1006 -450T976 -439T958 -411Q957 -407 957 37Q957 484 956 494Q945 643 831 747T554 852Q481 852 411 826Q301 786 232 696T154 494Q153 484 153 37Q153 -407 152 -411Q148 -428 135 -439T104 -450T73 -439T56 -410Q55 -406 55 49Q56 505 57 516Z"
                    />
                  </g>
                  <g transform="translate(368,-1051)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                  <g transform="translate(403,1151)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="bigcup_ab_745232433" viewBox="0 -1867.7 1111.5 3161.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 911Q58 926 71 938T103 950Q120 950 134 939T152 911Q153 907 153 463Q153 16 154 6Q165 -143 279 -247T556 -352Q716 -352 830 -248T956 6Q957 16 957 463Q957 907 958 911Q962 928 975 939T1006 950T1037 939T1054 911Q1055 906 1055 451Q1054 -5 1053 -16Q1029 -207 889 -328T555 -449Q363 -449 226 -331T62 -45Q57 -16 57 25T55 451Q55 906 56 911Z"
                  />
                  <g transform="translate(368,-1051)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                  <g transform="translate(403,1150)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="bot_2087963360" viewBox="0 -791.3 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M369 652Q369 653 370 655T372 658T375 662T379 665T384 667T391 668Q402 666 409 653V40H708Q723 32 723 20T708 0H71Q70 0 67 2T59 9T55 20T59 31T66 38T71 40H369V652Z"
                  />
                </g>
              </symbol>
              <symbol id="bullet_1823366303" viewBox="0 -576.1 500.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 251Q55 328 112 386T249 444T386 388T444 249Q444 171 388 113T250 55Q170 55 113 112T55 251Z"
                  />
                </g>
              </symbol>
              <symbol id="cap_2088096139" viewBox="0 -719.6 667.5 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M88 -21T75 -21T55 -7V200Q55 231 55 280Q56 414 60 428Q61 430 61 431Q77 500 152 549T332 598Q443 598 522 544T610 405Q611 399 611 194V-7Q604 -22 591 -22Q582 -22 572 -9L570 405Q563 433 556 449T529 485Q498 519 445 538T334 558Q251 558 179 518T96 401Q95 396 95 193V-7Q88 -21 75 -21Z"
                  />
                </g>
              </symbol>
              <symbol id="cdot_183307621" viewBox="0 -432.6 278.5 506.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M78 250Q78 274 95 292T138 310Q162 310 180 294T199 251Q199 226 182 208T139 190T96 207T78 250Z"
                  />
                </g>
              </symbol>
              <symbol id="chi_2087832827" viewBox="0 -576.1 626.5 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M576 -125Q576 -147 547 -175T487 -204H476Q394 -204 363 -157Q334 -114 293 26L284 59Q283 58 248 19T170 -66T92 -151T53 -191Q49 -194 43 -194Q36 -194 31 -189T25 -177T38 -154T151 -30L272 102L265 131Q189 405 135 405Q104 405 87 358Q86 351 68 351Q48 351 48 361Q48 369 56 386T89 423T148 442Q224 442 258 400Q276 375 297 320T330 222L341 180Q344 180 455 303T573 429Q579 431 582 431Q600 431 600 414Q600 407 587 392T477 270Q356 138 353 134L362 102Q392 -10 428 -89T490 -168Q504 -168 517 -156T536 -126Q539 -116 543 -115T557 -114T571 -115Q576 -118 576 -125Z"
                  />
                </g>
              </symbol>
              <symbol id="circ_177147074" viewBox="0 -576.1 500.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 251Q55 328 112 386T249 444T386 388T444 249Q444 171 388 113T250 55Q170 55 113 112T55 251ZM245 403Q188 403 142 361T96 250Q96 183 141 140T250 96Q284 96 313 109T354 135T375 160Q403 197 403 250Q403 313 360 358T245 403Z"
                  />
                </g>
              </symbol>
              <symbol id="coprod_ab_1722723006" viewBox="0 -1867.7 1278.5 3161.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M220 812Q220 813 218 819T214 829T208 840T199 853T185 866T166 878T140 887T107 893T66 896H56V950H552V896H542Q411 896 389 812L388 208V-396H889V812Q889 813 887 819T883 829T877 840T868 853T854 866T835 878T809 887T776 893T735 896H725V950H1221V896H1211Q1080 896 1058 812V-311Q1076 -396 1211 -396H1221V-450H56V-396H66Q195 -396 219 -314Q220 -312 220 -311V812Z"
                  />
                  <g transform="translate(452,-1051)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                  <g transform="translate(487,1151)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="cup_2088074399" viewBox="0 -719.6 667.5 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M591 598H592Q604 598 611 583V376Q611 345 611 296Q610 162 606 148Q605 146 605 145Q586 68 507 23T333 -22Q268 -22 209 -1T106 66T56 173Q55 180 55 384L56 585Q66 598 75 598Q85 598 95 585V378L96 172L98 162Q112 95 181 57T332 18Q415 18 487 58T570 175Q571 180 571 383V583Q579 598 591 598Z"
                  />
                </g>
              </symbol>
              <symbol id="dagger_1389219435" viewBox="0 -791.3 444.5 1152.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M182 675Q195 705 222 705Q234 705 243 700T253 691T263 675L262 655Q262 620 252 549T240 454V449Q250 451 288 461T346 472T377 461T389 431Q389 417 379 404T346 390Q327 390 288 401T243 412H240V405Q245 367 250 339T258 301T261 274T263 225Q263 124 255 -41T239 -213Q236 -216 222 -216H217Q206 -216 204 -212T200 -186Q199 -175 199 -168Q181 38 181 225Q181 265 182 280T191 327T204 405V412H201Q196 412 157 401T98 390Q76 390 66 403T55 431T65 458T98 472Q116 472 155 462T205 449Q204 452 204 460T201 490T193 547Q182 619 182 655V675Z"
                  />
                </g>
              </symbol>
              <symbol id="ddagger_2894567663" viewBox="0 -791.3 444.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M181 658Q181 705 222 705T263 658Q263 633 252 572T240 497Q240 496 241 496Q243 496 285 507T345 519Q365 519 376 508T388 478Q388 466 384 458T375 447T361 438H344Q318 438 282 448T241 459Q240 458 240 456Q240 449 251 384T263 297Q263 278 255 267T238 253T222 250T206 252T190 266T181 297Q181 323 192 383T204 458Q204 459 203 459Q198 459 162 449T101 438H84Q74 443 70 446T61 457T56 478Q56 497 67 508T99 519Q117 519 159 508T203 496Q204 496 204 499Q204 507 193 572T181 658ZM181 202Q181 249 222 249T263 202Q263 185 259 161T249 103T240 48V41H243Q248 41 287 52T346 63T377 52T389 22Q389 8 379 -5T346 -19Q327 -19 288 -8T243 3H240V-4Q243 -24 249 -58T259 -117T263 -158Q263 -177 255 -188T238 -202T222 -205T206 -203T190 -189T181 -158Q181 -141 185 -117T195 -59T204 -4V3H201Q196 3 157 -8T98 -19Q76 -19 66 -6T55 22T65 49T98 63Q117 63 156 52T201 41H204V48Q201 68 195 102T185 161T181 202Z"
                  />
                </g>
              </symbol>
              <symbol id="delta_1323076193" viewBox="0 -791.3 833.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M51 0Q46 4 46 7Q46 9 215 357T388 709Q391 716 416 716Q439 716 444 709Q447 705 616 357T786 7Q786 4 781 0H51ZM507 344L384 596L137 92L383 91H630Q630 93 507 344Z"
                  />
                </g>
              </symbol>
              <symbol id="delta_1323077121" viewBox="0 -863.1 451.5 1008.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M195 609Q195 656 227 686T302 717Q319 716 351 709T407 697T433 690Q451 682 451 662Q451 644 438 628T403 612Q382 612 348 641T288 671T249 657T235 628Q235 584 334 463Q401 379 401 292Q401 169 340 80T205 -10H198Q127 -10 83 36T36 153Q36 286 151 382Q191 413 252 434Q252 435 245 449T230 481T214 521T201 566T195 609ZM112 130Q112 83 136 55T204 27Q233 27 256 51T291 111T309 178T316 232Q316 267 309 298T295 344T269 400L259 396Q215 381 183 342T137 256T118 179T112 130Z"
                  />
                </g>
              </symbol>
              <symbol id="diamond_2948158269" viewBox="0 -576.1 500.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M242 486Q245 488 250 488Q256 488 258 486Q262 484 373 373T486 258T488 250T486 242T373 127T258 14Q256 12 250 12Q245 12 242 14Q237 16 127 126T14 242Q12 245 12 250T14 258Q16 263 126 373T242 486ZM439 250L250 439L61 250L250 61L439 250Z"
                  />
                </g>
              </symbol>
              <symbol id="div_2088172514" viewBox="0 -647.8 778.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M318 466Q318 500 339 518T386 537Q418 537 438 517T458 466Q458 438 440 417T388 396Q355 396 337 417T318 466ZM56 237T56 250T70 270H706Q721 262 721 250T706 230H70Q56 237 56 250ZM318 34Q318 68 339 86T386 105Q418 105 438 85T458 34Q458 6 440 -15T388 -36Q355 -36 337 -15T318 34Z"
                  />
                </g>
              </symbol>
              <symbol id="downarrow_2479725650" viewBox="0 -791.3 500.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M473 86Q483 86 483 67Q483 63 483 61T483 56T481 53T480 50T478 48T474 47T470 46T464 44Q428 35 391 14T316 -55T264 -168Q264 -170 263 -173T262 -180T261 -184Q259 -194 251 -194Q242 -194 238 -176T221 -121T180 -49Q169 -34 155 -21T125 2T95 20T67 33T44 42T27 47L21 49Q17 53 17 67Q17 87 28 87Q33 87 42 84Q158 52 223 -45L230 -55V312Q230 391 230 482T229 591Q229 662 231 676T243 693Q244 694 251 694Q264 692 270 679V-55L277 -45Q307 1 353 33T430 76T473 86Z"
                  />
                </g>
              </symbol>
              <symbol id="downarrow_2479726706" viewBox="0 -791.3 611.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M401 694Q412 694 422 681V375L423 70L435 81Q487 130 551 162Q564 170 570 170Q572 170 579 163V150Q579 138 577 135T564 126Q541 114 518 99T453 48T374 -46T318 -177Q313 -194 305 -194T293 -178T272 -119T225 -31Q158 70 46 126Q35 132 33 135T31 150V163Q38 170 40 170Q46 170 59 162Q122 131 176 81L188 70V375L189 681Q199 694 208 694Q219 694 228 680V352L229 25L238 12Q279 -42 305 -102Q344 -23 373 13L382 25V678Q387 692 401 694Z"
                  />
                </g>
              </symbol>
              <symbol id="epsilon_3526400827" viewBox="0 -576.1 406.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M227 -11Q149 -11 95 41T40 174Q40 262 87 322Q121 367 173 396T287 430Q289 431 329 431H367Q382 426 382 411Q382 385 341 385H325H312Q191 385 154 277L150 265H327Q340 256 340 246Q340 228 320 219H138V217Q128 187 128 143Q128 77 160 52T231 26Q258 26 284 36T326 57T343 68Q350 68 354 58T358 39Q358 36 357 35Q354 31 337 21T289 0T227 -11Z"
                  />
                </g>
              </symbol>
              <symbol id="equiv_1981568039" viewBox="0 -576.1 778.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 444Q56 457 70 464H707Q722 456 722 444Q722 430 706 424H72Q56 429 56 444ZM56 237T56 250T70 270H707Q722 262 722 250T707 230H70Q56 237 56 250ZM56 56Q56 71 72 76H706Q722 70 722 56Q722 44 707 36H70Q56 43 56 56Z"
                  />
                </g>
              </symbol>
              <symbol id="eta_2087567017" viewBox="0 -576.1 503.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M21 287Q22 290 23 295T28 317T38 348T53 381T73 411T99 433T132 442Q156 442 175 435T205 417T221 395T229 376L231 369Q231 367 232 367L243 378Q304 442 382 442Q436 442 469 415T503 336V326Q503 302 439 53Q381 -182 377 -189Q364 -216 332 -216Q319 -216 310 -208T299 -186Q299 -177 358 57L420 307Q423 322 423 345Q423 404 379 404H374Q288 404 229 303L222 291L189 157Q156 26 151 16Q138 -11 108 -11Q95 -11 87 -5T76 7T74 17Q74 30 114 189T154 366Q154 405 128 405Q107 405 92 377T68 316T57 280Q55 278 41 278H27Q21 284 21 287Z"
                  />
                </g>
              </symbol>
              <symbol id="exists_1947215609" viewBox="0 -791.3 556.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 661T56 674T70 694H487Q497 686 500 679V15Q497 10 487 1L279 0H70Q56 7 56 20T70 40H460V327H84Q70 334 70 347T84 367H460V654H70Q56 661 56 674Z"
                  />
                </g>
              </symbol>
              <symbol id="forall_4023651683" viewBox="0 -791.3 556.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M0 673Q0 684 7 689T20 694Q32 694 38 680T82 567L126 451H430L473 566Q483 593 494 622T512 668T519 685Q524 694 538 694Q556 692 556 674Q556 670 426 329T293 -15Q288 -22 278 -22T263 -15Q260 -11 131 328T0 673ZM414 410Q414 411 278 411T142 410L278 55L414 410Z"
                  />
                </g>
              </symbol>
              <symbol id="fracab_2392580300" viewBox="0 -1221.9 889.5 2085">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <g transform="translate(120,0)">
                    <rect stroke="none" width="649" height="60" x="0" y="220" />
                    <g transform="translate(60,676)">
                      <path
                        stroke-width="1"
                        d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                      />
                    </g>
                    <g transform="translate(110,-715)">
                      <path
                        stroke-width="1"
                        d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                      />
                    </g>
                  </g>
                </g>
              </symbol>
              <symbol id="gamma_1311936542" viewBox="0 -576.1 543.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M31 249Q11 249 11 258Q11 275 26 304T66 365T129 418T206 441Q233 441 239 440Q287 429 318 386T371 255Q385 195 385 170Q385 166 386 166L398 193Q418 244 443 300T486 391T508 430Q510 431 524 431H537Q543 425 543 422Q543 418 522 378T463 251T391 71Q385 55 378 6T357 -100Q341 -165 330 -190T303 -216Q286 -216 286 -188Q286 -138 340 32L346 51L347 69Q348 79 348 100Q348 257 291 317Q251 355 196 355Q148 355 108 329T51 260Q49 251 47 251Q45 249 31 249Z"
                  />
                </g>
              </symbol>
              <symbol id="gamma_1311937598" viewBox="0 -791.3 625.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M128 619Q121 626 117 628T101 631T58 634H25V680H554V676Q556 670 568 560T582 444V440H542V444Q542 445 538 478T523 545T492 598Q454 634 349 634H334Q264 634 249 633T233 621Q232 618 232 339L233 61Q240 54 245 52T270 48T333 46H360V0H348Q324 3 182 3Q51 3 36 0H25V46H58Q100 47 109 49T128 61V619Z"
                  />
                </g>
              </symbol>
              <symbol id="geq_2088123562" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M83 616Q83 624 89 630T99 636Q107 636 253 568T543 431T687 361Q694 356 694 346T687 331Q685 329 395 192L107 56H101Q83 58 83 76Q83 77 83 79Q82 86 98 95Q117 105 248 167Q326 204 378 228L626 346L360 472Q291 505 200 548Q112 589 98 597T83 616ZM84 -118Q84 -108 99 -98H678Q694 -104 694 -118Q694 -130 679 -138H98Q84 -131 84 -118Z"
                  />
                </g>
              </symbol>
              <symbol id="gg_193411929" viewBox="0 -647.8 1000.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 539T55 547T60 561T74 567Q81 567 207 498Q297 449 365 412Q633 265 636 261Q639 255 639 250Q639 241 626 232Q614 224 365 88Q83 -65 79 -66Q76 -67 73 -67Q65 -67 60 -61T55 -47Q55 -39 61 -33Q62 -33 95 -15T193 39T320 109L321 110H322L323 111H324L325 112L326 113H327L329 114H330L331 115H332L333 116L334 117H335L336 118H337L338 119H339L340 120L341 121H342L343 122H344L345 123H346L347 124L348 125H349L351 126H352L353 127H354L355 128L356 129H357L358 130H359L360 131H361L362 132L363 133H364L365 134H366L367 135H368L369 136H370L371 137L372 138H373L374 139H375L376 140L378 141L576 251Q63 530 62 533Q55 539 55 547ZM360 539T360 547T365 561T379 567Q386 567 512 498Q602 449 670 412Q938 265 941 261Q944 255 944 250Q944 241 931 232Q919 224 670 88Q388 -65 384 -66Q381 -67 378 -67Q370 -67 365 -61T360 -47Q360 -39 366 -33Q367 -33 400 -15T498 39T625 109L626 110H627L628 111H629L630 112L631 113H632L634 114H635L636 115H637L638 116L639 117H640L641 118H642L643 119H644L645 120L646 121H647L648 122H649L650 123H651L652 124L653 125H654L656 126H657L658 127H659L660 128L661 129H662L663 130H664L665 131H666L667 132L668 133H669L670 134H671L672 135H673L674 136H675L676 137L677 138H678L679 139H680L681 140L683 141L881 251Q368 530 367 533Q360 539 360 547Z"
                  />
                </g>
              </symbol>
              <symbol id="im_193412861" viewBox="0 -791.3 554.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M190 601Q161 601 137 587T97 553T71 512T55 477T48 463Q44 465 39 468L30 473L35 488Q73 594 106 636T199 685Q200 686 211 686Q250 686 326 652T417 617Q435 617 455 626T497 652T522 670Q532 660 532 654Q469 591 390 550L378 543L343 556Q223 601 190 601ZM378 208Q378 249 369 318T360 424Q360 430 360 439T361 451L362 462Q416 526 482 571L495 580L503 577L511 575L499 562Q442 502 442 465Q442 436 452 368T462 246Q462 169 442 128T385 56Q292 -26 195 -26Q150 -26 104 14L96 21L43 -16Q43 -15 43 -14T41 -10T38 0L48 13Q76 50 123 97L150 125Q154 131 159 131Q166 131 171 116T182 81T193 53Q199 43 216 33T261 22Q307 22 344 68Q378 113 378 208Z"
                  />
                </g>
              </symbol>
              <symbol id="in_193419230" viewBox="0 -647.8 667.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M84 250Q84 372 166 450T360 539Q361 539 377 539T419 540T469 540H568Q583 532 583 520Q583 511 570 501L466 500Q355 499 329 494Q280 482 242 458T183 409T147 354T129 306T124 272V270H568Q583 262 583 250T568 230H124V228Q124 207 134 177T167 112T231 48T328 7Q355 1 466 0H570Q583 -10 583 -20Q583 -32 568 -40H471Q464 -40 446 -40T417 -41Q262 -41 172 45Q84 127 84 250Z"
                  />
                </g>
              </symbol>
              <symbol id="infty_2224287093" viewBox="0 -576.1 1000.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 217Q55 305 111 373T254 442Q342 442 419 381Q457 350 493 303L507 284L514 294Q618 442 747 442Q833 442 888 374T944 214Q944 128 889 59T743 -11Q657 -11 580 50Q542 81 506 128L492 147L485 137Q381 -11 252 -11Q166 -11 111 57T55 217ZM907 217Q907 285 869 341T761 397Q740 397 720 392T682 378T648 359T619 335T594 310T574 285T559 263T548 246L543 238L574 198Q605 158 622 138T664 94T714 61T765 51Q827 51 867 100T907 217ZM92 214Q92 145 131 89T239 33Q357 33 456 193L425 233Q364 312 334 337Q285 380 233 380Q171 380 132 331T92 214Z"
                  />
                </g>
              </symbol>
              <symbol id="int_ab_3437240392" viewBox="0 -1724.2 1464.7 2730.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M114 -798Q132 -824 165 -824H167Q195 -824 223 -764T275 -600T320 -391T362 -164Q365 -143 367 -133Q439 292 523 655T645 1127Q651 1145 655 1157T672 1201T699 1257T733 1306T777 1346T828 1360Q884 1360 912 1325T944 1245Q944 1220 932 1205T909 1186T887 1183Q866 1183 849 1198T832 1239Q832 1287 885 1296L882 1300Q879 1303 874 1307T866 1313Q851 1323 833 1323Q819 1323 807 1311T775 1255T736 1139T689 936T633 628Q574 293 510 -5T410 -437T355 -629Q278 -862 165 -862Q125 -862 92 -831T55 -746Q55 -711 74 -698T112 -685Q133 -685 150 -700T167 -741Q167 -789 114 -798Z"
                  />
                  <g transform="translate(1060,1089)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                  <g transform="translate(556,-898)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="iota_170249034" viewBox="0 -576.1 354.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M139 -10Q111 -10 92 0T64 25T52 52T48 74Q48 89 55 109T85 199T135 375L137 384Q139 394 140 397T145 409T151 422T160 431T173 439T190 442Q202 442 213 435T225 410Q225 404 214 358T181 238T137 107Q126 74 126 54Q126 43 126 39T130 31T142 27H147Q206 27 255 78Q272 98 281 114T290 138T295 149T313 153Q321 153 324 153T329 152T332 149T332 143Q332 106 276 48T145 -10H139Z"
                  />
                </g>
              </symbol>
              <symbol id="kappa_1326819122" viewBox="0 -576.1 576.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M83 -11Q70 -11 62 -4T51 8T49 17Q49 30 96 217T147 414Q160 442 193 442Q205 441 213 435T223 422T225 412Q225 401 208 337L192 270Q193 269 208 277T235 292Q252 304 306 349T396 412T467 431Q489 431 500 420T512 391Q512 366 494 347T449 327Q430 327 418 338T405 368Q405 370 407 380L397 375Q368 360 315 315L253 266L240 257H245Q262 257 300 251T366 230Q422 203 422 150Q422 140 417 114T411 67Q411 26 437 26Q484 26 513 137Q516 149 519 151T535 153Q554 153 554 144Q554 121 527 64T457 -7Q447 -10 431 -10Q386 -10 360 17T333 90Q333 108 336 122T339 146Q339 170 320 186T271 209T222 218T185 221H180L155 122Q129 22 126 16Q113 -11 83 -11Z"
                  />
                </g>
              </symbol>
              <symbol id="lambda_79015454" viewBox="0 -791.3 583.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M166 673Q166 685 183 694H202Q292 691 316 644Q322 629 373 486T474 207T524 67Q531 47 537 34T546 15T551 6T555 2T556 -2T550 -11H482Q457 3 450 18T399 152L354 277L340 262Q327 246 293 207T236 141Q211 112 174 69Q123 9 111 -1T83 -12Q47 -12 47 20Q47 37 61 52T199 187Q229 216 266 252T321 306L338 322Q338 323 288 462T234 612Q214 657 183 657Q166 657 166 673Z"
                  />
                </g>
              </symbol>
              <symbol id="lambda_79016510" viewBox="0 -791.3 694.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M320 708Q326 716 340 716H348H355Q367 716 372 708Q374 706 423 547T523 226T575 62Q581 52 591 50T634 46H661V0H653Q644 3 532 3Q411 3 390 0H379V46H392Q464 46 464 65Q463 70 390 305T316 539L246 316Q177 95 177 84Q177 72 198 59T248 46H253V0H245Q230 3 130 3Q47 3 38 0H32V46H45Q112 51 127 91Q128 92 224 399T320 708Z"
                  />
                </g>
              </symbol>
              <symbol id="leftarrow_2463726907" viewBox="0 -647.8 1000.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M944 261T944 250T929 230H165Q167 228 182 216T211 189T244 152T277 96T303 25Q308 7 308 0Q308 -11 288 -11Q281 -11 278 -11T272 -7T267 2T263 21Q245 94 195 151T73 236Q58 242 55 247Q55 254 59 257T73 264Q121 283 158 314T215 375T247 434T264 480L267 497Q269 503 270 505T275 509T288 511Q308 511 308 500Q308 493 303 475Q293 438 278 406T246 352T215 315T185 287T165 270H929Q944 261 944 250Z"
                  />
                </g>
              </symbol>
              <symbol id="leftarrow_2463728091" viewBox="0 -647.8 1000.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M944 153Q944 140 929 133H318L328 123Q379 69 414 0Q419 -13 419 -17Q419 -24 399 -24Q388 -24 385 -23T377 -12Q332 77 253 144T72 237Q62 240 59 242T56 250T59 257T70 262T89 268T119 278T160 296Q303 366 377 512Q382 522 385 523T401 525Q419 524 419 515Q419 510 414 500Q379 431 328 377L318 367H929Q944 359 944 347Q944 336 930 328L602 327H274L264 319Q225 289 147 250Q148 249 165 241T210 217T264 181L274 173H930Q931 172 933 171T936 169T938 167T941 164T942 162T943 158T944 153Z"
                  />
                </g>
              </symbol>
              <symbol id="leftrightarrow_2294107771" viewBox="0 -647.8 1000.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M263 479Q267 501 271 506T288 511Q308 511 308 500Q308 493 303 475Q293 438 278 406T246 352T215 315T185 287T165 270H835Q729 349 696 475Q691 493 691 500Q691 511 711 511Q720 511 723 510T729 506T732 497T735 481T743 456Q765 389 816 336T935 261Q944 258 944 250Q944 244 939 241T915 231T877 212Q836 186 806 152T761 85T740 35T732 4Q730 -6 727 -8T711 -11Q691 -11 691 0Q691 7 696 25Q728 151 835 230H165Q167 228 182 216T211 189T244 152T277 96T303 25Q308 7 308 0Q308 -11 288 -11Q281 -11 278 -11T272 -7T267 2T263 21Q245 94 195 151T73 236Q58 242 55 247Q55 254 59 257T73 264Q144 292 194 349T263 479Z"
                  />
                </g>
              </symbol>
              <symbol id="leftrightarrow_2294108699" viewBox="0 -647.8 1000.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M308 524Q318 526 323 526Q340 526 340 514Q340 507 336 499Q326 476 314 454T292 417T274 391T260 374L255 368Q255 367 500 367Q744 367 744 368L739 374Q734 379 726 390T707 416T685 453T663 499Q658 511 658 515Q658 525 680 525Q687 524 690 523T695 519T701 507Q766 359 902 287Q921 276 939 269T961 259T966 250Q966 246 965 244T960 240T949 236T930 228T902 213Q763 137 701 -7Q697 -16 695 -19T690 -23T680 -25Q658 -25 658 -15Q658 -11 663 1Q673 24 685 46T707 83T725 109T739 126L744 132Q744 133 500 133Q255 133 255 132L260 126Q265 121 273 110T292 84T314 47T336 1Q341 -11 341 -15Q341 -25 319 -25Q312 -24 309 -23T304 -19T298 -7Q233 141 97 213Q83 221 70 227T51 235T41 239T35 243T34 250T35 256T40 261T51 265T70 273T97 287Q235 363 299 509Q305 522 308 524ZM792 319L783 327H216Q183 294 120 256L110 250L120 244Q173 212 207 181L216 173H783L792 181Q826 212 879 244L889 250L879 256Q826 288 792 319Z"
                  />
                </g>
              </symbol>
              <symbol id="leftxright_1060968992" viewBox="0 -863.1 1129.5 1223.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path stroke-width="1" d="M118 -250V750H255V710H158V-210H255V-250H118Z" />
                  <g transform="translate(278,0)">
                    <path
                      stroke-width="1"
                      d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                    />
                  </g>
                  <g transform="translate(851,0)">
                    <path stroke-width="1" d="M22 710V750H159V-250H22V-210H119V710H22Z" />
                  </g>
                </g>
              </symbol>
              <symbol id="leftxright_1282332352" viewBox="0 -863.1 1573.5 1223.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M434 -231Q434 -244 428 -250H410Q281 -250 230 -184Q225 -177 222 -172T217 -161T213 -148T211 -133T210 -111T209 -84T209 -47T209 0Q209 21 209 53Q208 142 204 153Q203 154 203 155Q189 191 153 211T82 231Q71 231 68 234T65 250T68 266T82 269Q116 269 152 289T203 345Q208 356 208 377T209 529V579Q209 634 215 656T244 698Q270 724 324 740Q361 748 377 749Q379 749 390 749T408 750H428Q434 744 434 732Q434 719 431 716Q429 713 415 713Q362 710 332 689T296 647Q291 634 291 499V417Q291 370 288 353T271 314Q240 271 184 255L170 250L184 245Q202 239 220 230T262 196T290 137Q291 131 291 1Q291 -134 296 -147Q306 -174 339 -192T415 -213Q429 -213 431 -216Q434 -219 434 -231Z"
                  />
                  <g transform="translate(500,0)">
                    <path
                      stroke-width="1"
                      d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                    />
                  </g>
                  <g transform="translate(1073,0)">
                    <path
                      stroke-width="1"
                      d="M65 731Q65 745 68 747T88 750Q171 750 216 725T279 670Q288 649 289 635T291 501Q292 362 293 357Q306 312 345 291T417 269Q428 269 431 266T434 250T431 234T417 231Q380 231 345 210T298 157Q293 143 292 121T291 -28V-79Q291 -134 285 -156T256 -198Q202 -250 89 -250Q71 -250 68 -247T65 -230Q65 -224 65 -223T66 -218T69 -214T77 -213Q91 -213 108 -210T146 -200T183 -177T207 -139Q208 -134 209 3L210 139Q223 196 280 230Q315 247 330 250Q305 257 280 270Q225 304 212 352L210 362L209 498Q208 635 207 640Q195 680 154 696T77 713Q68 713 67 716T65 731Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="leftxright_308942087" viewBox="0 -863.1 1351.5 1223.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M94 250Q94 319 104 381T127 488T164 576T202 643T244 695T277 729T302 750H315H319Q333 750 333 741Q333 738 316 720T275 667T226 581T184 443T167 250T184 58T225 -81T274 -167T316 -220T333 -241Q333 -250 318 -250H315H302L274 -226Q180 -141 137 -14T94 250Z"
                  />
                  <g transform="translate(389,0)">
                    <path
                      stroke-width="1"
                      d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                    />
                  </g>
                  <g transform="translate(962,0)">
                    <path
                      stroke-width="1"
                      d="M60 749L64 750Q69 750 74 750H86L114 726Q208 641 251 514T294 250Q294 182 284 119T261 12T224 -76T186 -143T145 -194T113 -227T90 -246Q87 -249 86 -250H74Q66 -250 63 -250T58 -247T55 -238Q56 -237 66 -225Q221 -64 221 250T66 725Q56 737 55 738Q55 746 60 749Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="leftxright_791668582" viewBox="0 -863.1 1129.5 1223.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M139 -249H137Q125 -249 119 -235V251L120 737Q130 750 139 750Q152 750 159 735V-235Q151 -249 141 -249H139Z"
                  />
                  <g transform="translate(278,0)">
                    <path
                      stroke-width="1"
                      d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                    />
                  </g>
                  <g transform="translate(851,0)">
                    <path
                      stroke-width="1"
                      d="M139 -249H137Q125 -249 119 -235V251L120 737Q130 750 139 750Q152 750 159 735V-235Q151 -249 141 -249H139Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="leq_2088123777" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M674 636Q682 636 688 630T694 615T687 601Q686 600 417 472L151 346L399 228Q687 92 691 87Q694 81 694 76Q694 58 676 56H670L382 192Q92 329 90 331Q83 336 83 348Q84 359 96 365Q104 369 382 500T665 634Q669 636 674 636ZM84 -118Q84 -108 99 -98H678Q694 -104 694 -118Q694 -130 679 -138H98Q84 -131 84 -118Z"
                  />
                </g>
              </symbol>
              <symbol id="lim_arightarrowbx_3542561616" viewBox="0 -791.3 2129.7 1654.5">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M42 46H56Q95 46 103 60V68Q103 77 103 91T103 124T104 167T104 217T104 272T104 329Q104 366 104 407T104 482T104 542T103 586T103 603Q100 622 89 628T44 637H26V660Q26 683 28 683L38 684Q48 685 67 686T104 688Q121 689 141 690T171 693T182 694H185V379Q185 62 186 60Q190 52 198 49Q219 46 247 46H263V0H255L232 1Q209 2 183 2T145 3T107 3T57 1L34 0H26V46H42Z"
                  />
                  <path
                    stroke-width="1"
                    d="M69 609Q69 637 87 653T131 669Q154 667 171 652T188 609Q188 579 171 564T129 549Q104 549 87 564T69 609ZM247 0Q232 3 143 3Q132 3 106 3T56 1L34 0H26V46H42Q70 46 91 49Q100 53 102 60T104 102V205V293Q104 345 102 359T88 378Q74 385 41 385H30V408Q30 431 32 431L42 432Q52 433 70 434T106 436Q123 437 142 438T171 441T182 442H185V62Q190 52 197 50T232 46H255V0H247Z"
                    transform="translate(278,0)"
                  />
                  <path
                    stroke-width="1"
                    d="M41 46H55Q94 46 102 60V68Q102 77 102 91T102 122T103 161T103 203Q103 234 103 269T102 328V351Q99 370 88 376T43 385H25V408Q25 431 27 431L37 432Q47 433 65 434T102 436Q119 437 138 438T167 441T178 442H181V402Q181 364 182 364T187 369T199 384T218 402T247 421T285 437Q305 442 336 442Q351 442 364 440T387 434T406 426T421 417T432 406T441 395T448 384T452 374T455 366L457 361L460 365Q463 369 466 373T475 384T488 397T503 410T523 422T546 432T572 439T603 442Q729 442 740 329Q741 322 741 190V104Q741 66 743 59T754 49Q775 46 803 46H819V0H811L788 1Q764 2 737 2T699 3Q596 3 587 0H579V46H595Q656 46 656 62Q657 64 657 200Q656 335 655 343Q649 371 635 385T611 402T585 404Q540 404 506 370Q479 343 472 315T464 232V168V108Q464 78 465 68T468 55T477 49Q498 46 526 46H542V0H534L510 1Q487 2 460 2T422 3Q319 3 310 0H302V46H318Q379 46 379 62Q380 64 380 200Q379 335 378 343Q372 371 358 385T334 402T308 404Q263 404 229 370Q202 343 195 315T187 232V168V108Q187 78 188 68T191 55T200 49Q221 46 249 46H265V0H257L234 1Q210 2 183 2T145 3Q42 3 33 0H25V46H41Z"
                    transform="translate(557,0)"
                  />
                  <g transform="translate(2,-660)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                    <g transform="translate(374,0)">
                      <path
                        stroke-width="1"
                        transform="scale(0.707)"
                        d="M56 237T56 250T70 270H835Q719 357 692 493Q692 494 692 496T691 499Q691 511 708 511H711Q720 511 723 510T729 506T732 497T735 481T743 456Q765 389 816 336T935 261Q944 258 944 250Q944 244 939 241T915 231T877 212Q836 186 806 152T761 85T740 35T732 4Q730 -6 727 -8T711 -11Q691 -11 691 0Q691 7 696 25Q728 151 835 230H70Q56 237 56 250Z"
                      />
                    </g>
                    <g transform="translate(1081,0)">
                      <path
                        stroke-width="1"
                        transform="scale(0.707)"
                        d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                      />
                    </g>
                  </g>
                  <g transform="translate(1557,0)">
                    <path
                      stroke-width="1"
                      d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="ll_193412985" viewBox="0 -647.8 1000.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M639 -48Q639 -54 634 -60T619 -67H618Q612 -67 536 -26Q430 33 329 88Q61 235 59 239Q56 243 56 250T59 261Q62 266 336 415T615 567L619 568Q622 567 625 567Q639 562 639 548Q639 540 633 534Q632 532 374 391L117 250L374 109Q632 -32 633 -34Q639 -40 639 -48ZM944 -48Q944 -54 939 -60T924 -67H923Q917 -67 841 -26Q735 33 634 88Q366 235 364 239Q361 243 361 250T364 261Q367 266 641 415T920 567L924 568Q927 567 930 567Q944 562 944 548Q944 540 938 534Q937 532 679 391L422 250L679 109Q937 -32 938 -34Q944 -40 944 -48Z"
                  />
                </g>
              </symbol>
              <symbol id="mp_193426244" viewBox="0 -576.1 778.5 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 467T56 480T70 500H707Q722 492 722 480T707 460H409V187H707Q722 179 722 167Q722 154 707 147H409V0V-93Q409 -144 406 -155T389 -166Q376 -166 372 -155T368 -105Q368 -96 368 -62T369 -2V147H70Q56 154 56 167T70 187H369V460H70Q56 467 56 480Z"
                  />
                </g>
              </symbol>
              <symbol id="mu_193423105" viewBox="0 -576.1 603.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M58 -216Q44 -216 34 -208T23 -186Q23 -176 96 116T173 414Q186 442 219 442Q231 441 239 435T249 423T251 413Q251 401 220 279T187 142Q185 131 185 107V99Q185 26 252 26Q261 26 270 27T287 31T302 38T315 45T327 55T338 65T348 77T356 88T365 100L372 110L408 253Q444 395 448 404Q461 431 491 431Q504 431 512 424T523 412T525 402L449 84Q448 79 448 68Q448 43 455 35T476 26Q485 27 496 35Q517 55 537 131Q543 151 547 152Q549 153 557 153H561Q580 153 580 144Q580 138 575 117T555 63T523 13Q510 0 491 -8Q483 -10 467 -10Q446 -10 429 -4T402 11T385 29T376 44T374 51L368 45Q362 39 350 30T324 12T288 -4T246 -11Q199 -11 153 12L129 -85Q108 -167 104 -180T92 -202Q76 -216 58 -216Z"
                  />
                </g>
              </symbol>
              <symbol id="ne_193405746" viewBox="0 -791.3 778.5 1152.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M166 -215T159 -215T147 -212T141 -204T139 -197Q139 -190 144 -183L306 133H70Q56 140 56 153Q56 168 72 173H327L406 327H72Q56 332 56 347Q56 360 70 367H426Q597 702 602 707Q605 716 618 716Q625 716 630 712T636 703T638 696Q638 692 471 367H707Q722 359 722 347Q722 336 708 328L451 327L371 173H708Q722 163 722 153Q722 140 707 133H351Q175 -210 170 -212Q166 -215 159 -215Z"
                  />
                </g>
              </symbol>
              <symbol id="neg_2087624725" viewBox="0 -432.6 667.5 506.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 323T56 336T70 356H596Q603 353 611 343V102Q598 89 591 89Q587 89 584 90T579 94T575 98T572 102L571 209V316H70Q56 323 56 336Z"
                  />
                </g>
              </symbol>
              <symbol id="ni_193418046" viewBox="0 -647.8 667.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M83 520Q83 532 98 540H195Q202 540 220 540T249 541Q404 541 494 455Q582 374 582 250Q582 165 539 99T434 0T304 -39Q297 -40 195 -40H98Q83 -32 83 -20Q83 -10 96 0H200Q311 1 337 6Q369 14 401 28Q422 39 445 55Q484 85 508 127T537 191T542 228V230H98Q84 237 84 250T98 270H542V272Q542 280 539 295T527 336T497 391T445 445Q422 461 401 472Q386 479 374 483T347 491T325 495T298 498T273 499T239 500T200 500L96 501Q83 511 83 520Z"
                  />
                </g>
              </symbol>
              <symbol id="notin_1650680875" viewBox="0 -791.3 667.5 1152.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M196 25Q84 109 84 250Q84 372 166 450T360 539Q361 539 375 539T413 540T460 540L547 707Q550 716 563 716Q570 716 575 712T581 703T583 696T505 540H568Q583 532 583 520Q583 511 570 501L484 500L366 270H568Q583 262 583 250T568 230H346L247 38Q284 16 328 7Q355 1 466 0H570Q583 -10 583 -20Q583 -32 568 -40H471Q464 -40 447 -40T419 -41Q304 -41 228 3Q117 -211 115 -212Q111 -215 104 -215T92 -212T86 -204T84 -197Q84 -190 89 -183L196 25ZM214 61L301 230H124V228Q124 196 147 147T214 61ZM321 270L440 500Q353 499 329 494Q280 482 242 458T183 409T147 354T129 306T124 272V270H321Z"
                  />
                </g>
              </symbol>
              <symbol id="nu_193423138" viewBox="0 -576.1 530.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M74 431Q75 431 146 436T219 442Q231 442 231 434Q231 428 185 241L137 51H140L150 55Q161 59 177 67T214 86T261 119T312 165Q410 264 445 394Q458 442 496 442Q509 442 519 434T530 411Q530 390 516 352T469 262T388 162T267 70T106 5Q81 -2 71 -2Q66 -2 59 -1T51 1Q45 5 45 11Q45 13 88 188L132 364Q133 377 125 380T86 385H65Q59 391 59 393T61 412Q65 431 74 431Z"
                  />
                </g>
              </symbol>
              <symbol id="odot_183307497" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 250Q56 394 156 488T384 583Q530 583 626 485T722 250Q722 110 625 14T390 -83Q249 -83 153 14T56 250ZM682 250Q682 322 649 387T546 497T381 542Q272 542 184 459T95 250Q95 132 178 45T389 -42Q515 -42 598 45T682 250ZM311 250Q311 285 332 304T375 328Q376 328 382 328T392 329Q424 326 445 305T466 250Q466 217 445 195T389 172Q354 172 333 195T311 250Z"
                  />
                </g>
              </symbol>
              <symbol id="oint_ab_1759784839" viewBox="0 -1724.2 1464.7 2730.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <g transform="translate(0,1)">
                    <path
                      stroke-width="1"
                      d="M114 -798Q132 -824 165 -824H167Q195 -824 223 -764T275 -600T320 -391T362 -164Q365 -143 367 -133Q382 -52 390 2Q314 40 276 99Q230 167 230 249Q230 363 305 436T484 519H494L503 563Q587 939 632 1087T727 1298Q774 1360 828 1360Q884 1360 912 1325T944 1245Q944 1220 932 1205T909 1186T887 1183Q866 1183 849 1198T832 1239Q832 1287 885 1296L882 1300Q879 1303 874 1307T866 1313Q851 1323 833 1323Q766 1323 688 929Q662 811 610 496Q770 416 770 249Q770 147 701 68T516 -21H506L497 -65Q407 -464 357 -623T237 -837Q203 -862 165 -862Q125 -862 92 -831T55 -746Q55 -711 74 -698T112 -685Q133 -685 150 -700T167 -741Q167 -789 114 -798ZM480 478Q460 478 435 470T380 444T327 401T287 335T271 249Q271 124 375 56L397 43L431 223L485 478H480ZM519 20Q545 20 578 33T647 72T706 144T730 249Q730 383 603 455Q603 454 597 421T582 343T569 276Q516 22 515 20H519Z"
                    />
                  </g>
                  <g transform="translate(1060,1088)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                  <g transform="translate(556,-898)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="omega_1309716888" viewBox="0 -576.1 622.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M495 384Q495 406 514 424T555 443Q574 443 589 425T604 364Q604 334 592 278T555 155T483 38T377 -11Q297 -11 267 66Q266 68 260 61Q201 -11 125 -11Q15 -11 15 139Q15 230 56 325T123 434Q135 441 147 436Q160 429 160 418Q160 406 140 379T94 306T62 208Q61 202 61 187Q61 124 85 100T143 76Q201 76 245 129L253 137V156Q258 297 317 297Q348 297 348 261Q348 243 338 213T318 158L308 135Q309 133 310 129T318 115T334 97T358 83T393 76Q456 76 501 148T546 274Q546 305 533 325T508 357T495 384Z"
                  />
                </g>
              </symbol>
              <symbol id="omega_1309717944" viewBox="0 -791.3 722.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 454Q55 503 75 546T127 617T197 665T272 695T337 704H352Q396 704 404 703Q527 687 596 615T666 454Q666 392 635 330T559 200T499 83V80H543Q589 81 600 83T617 93Q622 102 629 135T636 172L637 177H677V175L660 89Q645 3 644 2V0H552H488Q461 0 456 3T451 20Q451 89 499 235T548 455Q548 512 530 555T483 622T424 656T361 668Q332 668 303 658T243 626T193 560T174 456Q174 380 222 233T270 20Q270 7 263 0H77V2Q76 3 61 89L44 175V177H84L85 172Q85 171 88 155T96 119T104 93Q109 86 120 84T178 80H222V83Q206 132 162 199T87 329T55 454Z"
                  />
                </g>
              </symbol>
              <symbol id="ominus_1906208922" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 250Q56 394 156 488T384 583Q530 583 626 485T722 250Q722 110 625 14T390 -83Q249 -83 153 14T56 250ZM681 278Q669 385 591 463T381 542Q283 542 196 471T96 278V270H681V278ZM275 -42T388 -42T585 32T681 222V230H96V222Q108 107 191 33Z"
                  />
                </g>
              </symbol>
              <symbol id="oplus_2140408172" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 250Q56 394 156 488T384 583Q530 583 626 485T722 250Q722 110 625 14T390 -83Q249 -83 153 14T56 250ZM364 542Q308 539 251 509T148 418T96 278V270H369V542H364ZM681 278Q675 338 650 386T592 462T522 509T458 535T412 542H409V270H681V278ZM96 222Q104 150 139 95T219 12T302 -29T366 -42H369V230H96V222ZM681 222V230H409V-42H412Q429 -42 456 -36T521 -10T590 37T649 113T681 222Z"
                  />
                </g>
              </symbol>
              <symbol id="oslash_1351473011" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 250Q56 394 156 488T384 583Q530 583 626 485T722 250Q722 110 625 14T390 -83Q249 -83 153 14T56 250ZM582 471Q581 472 571 480T556 491T539 502T517 514T491 525T460 534T424 539T381 542Q272 542 184 460T95 251Q95 198 113 150T149 80L167 58L582 471ZM388 -42Q513 -42 597 44T682 250Q682 363 610 442L196 29Q209 16 229 2T295 -27T388 -42Z"
                  />
                </g>
              </symbol>
              <symbol id="otimes_2531166928" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 250Q56 394 156 488T384 583Q530 583 626 485T722 250Q722 110 625 14T390 -83Q249 -83 153 14T56 250ZM582 471Q531 510 496 523Q446 542 381 542Q324 542 272 519T196 471L389 278L485 375L582 471ZM167 442Q95 362 95 250Q95 137 167 58L359 250L167 442ZM610 58Q682 138 682 250Q682 363 610 442L418 250L610 58ZM196 29Q209 16 230 2T295 -27T388 -42Q409 -42 429 -40T465 -33T496 -23T522 -11T544 1T561 13T574 22T582 29L388 222L196 29Z"
                  />
                </g>
              </symbol>
              <symbol id="partial_1646186250" viewBox="0 -791.3 567.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M202 508Q179 508 169 520T158 547Q158 557 164 577T185 624T230 675T301 710L333 715H345Q378 715 384 714Q447 703 489 661T549 568T566 457Q566 362 519 240T402 53Q321 -22 223 -22Q123 -22 73 56Q42 102 42 148V159Q42 276 129 370T322 465Q383 465 414 434T455 367L458 378Q478 461 478 515Q478 603 437 639T344 676Q266 676 223 612Q264 606 264 572Q264 547 246 528T202 508ZM430 306Q430 372 401 400T333 428Q270 428 222 382Q197 354 183 323T150 221Q132 149 132 116Q132 21 232 21Q244 21 250 22Q327 35 374 112Q389 137 409 196T430 306Z"
                  />
                </g>
              </symbol>
              <symbol id="phi_2087833096" viewBox="0 -791.3 596.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M409 688Q413 694 421 694H429H442Q448 688 448 686Q448 679 418 563Q411 535 404 504T392 458L388 442Q388 441 397 441T429 435T477 418Q521 397 550 357T579 260T548 151T471 65T374 11T279 -10H275L251 -105Q245 -128 238 -160Q230 -192 227 -198T215 -205H209Q189 -205 189 -198Q189 -193 211 -103L234 -11Q234 -10 226 -10Q221 -10 206 -8T161 6T107 36T62 89T43 171Q43 231 76 284T157 370T254 422T342 441Q347 441 348 445L378 567Q409 686 409 688ZM122 150Q122 116 134 91T167 53T203 35T237 27H244L337 404Q333 404 326 403T297 395T255 379T211 350T170 304Q152 276 137 237Q122 191 122 150ZM500 282Q500 320 484 347T444 385T405 400T381 404H378L332 217L284 29Q284 27 285 27Q293 27 317 33T357 47Q400 66 431 100T475 170T494 234T500 282Z"
                  />
                </g>
              </symbol>
              <symbol id="phi_2087834152" viewBox="0 -791.3 722.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M312 622Q310 623 307 625T303 629T297 631T286 634T270 635T246 636T211 637H184V683H196Q220 680 361 680T526 683H538V637H511Q468 637 447 635T422 631T411 622V533L425 531Q525 519 595 466T665 342Q665 301 642 267T583 209T506 172T425 152L411 150V61Q417 55 421 53T447 48T511 46H538V0H526Q502 3 361 3T196 0H184V46H211Q231 46 245 46T270 47T286 48T297 51T303 54T307 57T312 61V150H310Q309 151 289 153T232 166T160 195Q149 201 136 210T103 238T69 284T56 342Q56 414 128 467T294 530Q309 532 310 533H312V622ZM170 342Q170 207 307 188H312V495H309Q301 495 282 491T231 469T186 423Q170 389 170 342ZM415 188Q487 199 519 236T551 342Q551 384 539 414T507 459T470 481T434 491T415 495H410V188H415Z"
                  />
                </g>
              </symbol>
              <symbol id="pi_193417920" viewBox="0 -791.3 750.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M128 619Q121 626 117 628T101 631T58 634H25V680H724V634H691Q651 633 640 631T622 619V61Q628 51 639 49T691 46H724V0H713Q692 3 569 3Q434 3 425 0H414V46H447Q489 47 498 49T517 61V634H232V348L233 61Q239 51 250 49T302 46H335V0H324Q303 3 180 3Q45 3 36 0H25V46H58Q100 47 109 49T128 61V619Z"
                  />
                </g>
              </symbol>
              <symbol id="pi_193418976" viewBox="0 -576.1 573.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M132 -11Q98 -11 98 22V33L111 61Q186 219 220 334L228 358H196Q158 358 142 355T103 336Q92 329 81 318T62 297T53 285Q51 284 38 284Q19 284 19 294Q19 300 38 329T93 391T164 429Q171 431 389 431Q549 431 553 430Q573 423 573 402Q573 371 541 360Q535 358 472 358H408L405 341Q393 269 393 222Q393 170 402 129T421 65T431 37Q431 20 417 5T381 -10Q370 -10 363 -7T347 17T331 77Q330 86 330 121Q330 170 339 226T357 318T367 358H269L268 354Q268 351 249 275T206 114T175 17Q164 -11 132 -11Z"
                  />
                </g>
              </symbol>
              <symbol id="pm_193414628" viewBox="0 -791.3 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 320T56 333T70 353H369V502Q369 651 371 655Q376 666 388 666Q402 666 405 654T409 596V500V353H707Q722 345 722 333Q722 320 707 313H409V40H707Q722 32 722 20T707 0H70Q56 7 56 20T70 40H369V313H70Q56 320 56 333Z"
                  />
                </g>
              </symbol>
              <symbol id="prec_176608541" viewBox="0 -647.8 778.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M84 249Q84 262 91 266T117 270Q120 270 126 270T137 269Q388 273 512 333T653 512Q657 539 676 539Q685 538 689 532T694 520V515Q689 469 672 431T626 366T569 320T500 286T435 265T373 249Q379 248 404 242T440 233T477 221T533 199Q681 124 694 -17Q694 -41 674 -41Q658 -41 653 -17Q646 41 613 84T533 154T418 197T284 220T137 229H114Q104 229 98 230T88 235T84 249Z"
                  />
                </g>
              </symbol>
              <symbol id="preceq_3517413801" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M84 346Q84 359 91 363T117 367Q120 367 126 367T137 366Q388 370 512 430T653 609Q657 636 676 636Q685 635 689 629T694 618V612Q689 566 672 528T626 463T569 417T500 383T435 362T373 346Q379 345 404 339T440 330T477 318T533 296Q592 266 630 223T681 145T694 78Q694 57 674 57Q662 57 657 67T652 92T640 135T606 191Q500 320 137 326H114Q104 326 98 327T88 332T84 346ZM84 -131T84 -118T98 -98H679Q694 -106 694 -118T679 -138H98Q84 -131 84 -118Z"
                  />
                </g>
              </symbol>
              <symbol id="prod_ab_1740866194" viewBox="0 -1867.7 1278.5 3161.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M220 812Q220 813 218 819T214 829T208 840T199 853T185 866T166 878T140 887T107 893T66 896H56V950H1221V896H1211Q1080 896 1058 812V-311Q1076 -396 1211 -396H1221V-450H725V-396H735Q864 -396 888 -314Q889 -312 889 -311V896H388V292L389 -311Q405 -396 542 -396H552V-450H56V-396H66Q195 -396 219 -314Q220 -312 220 -311V812Z"
                  />
                  <g transform="translate(452,-1051)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                  <g transform="translate(487,1151)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="psi_2087862643" viewBox="0 -791.3 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M340 622Q338 623 335 625T331 629T325 631T314 634T298 635T274 636T239 637H212V683H224Q248 680 389 680T554 683H566V637H539Q479 637 464 635T439 622L438 407Q438 192 439 192Q443 193 449 195T474 207T507 232T536 276T557 344Q560 365 562 417T573 493Q587 536 620 544Q627 546 671 546H715L722 540V515Q714 509 708 509Q680 505 671 476T658 392T644 307Q599 177 451 153L438 151V106L439 61Q446 54 451 52T476 48T539 46H566V0H554Q530 3 389 3T224 0H212V46H239Q259 46 273 46T298 47T314 48T325 51T331 54T335 57T340 61V151Q126 178 117 406Q115 503 69 509Q55 509 55 526Q55 541 59 543T86 546H107H120Q150 546 161 543T184 528Q198 514 204 493Q212 472 213 420T226 316T272 230Q287 216 303 207T330 194L339 192Q340 192 340 407V622Z"
                  />
                </g>
              </symbol>
              <symbol id="psi_2087863571" viewBox="0 -791.3 651.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M161 441Q202 441 226 417T250 358Q250 338 218 252T187 127Q190 85 214 61Q235 43 257 37Q275 29 288 29H289L371 360Q455 691 456 692Q459 694 472 694Q492 694 492 687Q492 678 411 356Q329 28 329 27T335 26Q421 26 498 114T576 278Q576 302 568 319T550 343T532 361T524 384Q524 405 541 424T583 443Q602 443 618 425T634 366Q634 337 623 288T605 220Q573 125 492 57T329 -11H319L296 -104Q272 -198 272 -199Q270 -205 252 -205H239Q233 -199 233 -197Q233 -192 256 -102T279 -9Q272 -8 265 -8Q106 14 106 139Q106 174 139 264T173 379Q173 380 173 381Q173 390 173 393T169 400T158 404H154Q131 404 112 385T82 344T65 302T57 280Q55 278 41 278H27Q21 284 21 287Q21 299 34 333T82 404T161 441Z"
                  />
                </g>
              </symbol>
              <symbol id="re_193404814" viewBox="0 -791.3 828.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M27 496Q31 569 102 627T234 685Q236 685 241 685T251 686Q287 686 318 672T367 638T399 598T418 564L423 550Q424 554 434 567T463 601T505 639T561 671T626 685Q672 685 688 659T710 572Q713 533 721 523T766 513Q781 513 787 514T794 516Q796 512 798 509T801 504T802 501T787 493Q702 461 624 401L607 389Q655 383 688 358L697 352V342Q699 330 699 297Q704 209 710 173T734 103Q751 69 765 69Q769 69 806 83L824 90V74Q823 73 759 24T693 -26Q692 -26 660 32L628 90L629 111Q631 159 631 177Q631 278 614 300Q584 340 523 340Q500 340 467 333T431 325Q429 325 429 322Q428 321 426 308T420 275T410 230T392 178T366 125L358 112L342 99Q306 70 269 38T213 -10T193 -26Q192 -26 163 0T116 26Q82 26 50 -8L42 -16L35 -8L27 0L35 10Q43 21 58 38T104 80T158 106Q179 106 218 65L235 48Q238 48 255 60T295 99T329 158Q352 231 352 359Q352 555 242 614Q210 628 187 628Q140 628 116 600T91 548Q91 522 138 464T185 382V376Q185 345 158 313T103 263L76 246Q74 244 64 253L54 260L65 267Q91 285 100 302Q111 318 111 337Q111 355 69 410T27 496ZM562 628Q504 628 443 507L435 491L436 479Q437 471 437 446Q437 396 432 351L529 389L602 426Q673 462 673 463H672Q644 470 637 483T622 553Q608 628 562 628Z"
                  />
                </g>
              </symbol>
              <symbol id="rho_2087900556" viewBox="0 -576.1 517.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M58 -216Q25 -216 23 -186Q23 -176 73 26T127 234Q143 289 182 341Q252 427 341 441Q343 441 349 441T359 442Q432 442 471 394T510 276Q510 219 486 165T425 74T345 13T266 -10H255H248Q197 -10 165 35L160 41L133 -71Q108 -168 104 -181T92 -202Q76 -216 58 -216ZM424 322Q424 359 407 382T357 405Q322 405 287 376T231 300Q217 269 193 170L176 102Q193 26 260 26Q298 26 334 62Q367 92 389 158T418 266T424 322Z"
                  />
                </g>
              </symbol>
              <symbol id="rightarrow_3982088320" viewBox="0 -647.8 1000.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M580 514Q580 525 596 525Q601 525 604 525T609 525T613 524T615 523T617 520T619 517T622 512Q659 438 720 381T831 300T927 263Q944 258 944 250T935 239T898 228T840 204Q696 134 622 -12Q618 -21 615 -22T600 -24Q580 -24 580 -17Q580 -13 585 0Q620 69 671 123L681 133H70Q56 140 56 153Q56 168 72 173H725L735 181Q774 211 852 250Q851 251 834 259T789 283T735 319L725 327H72Q56 332 56 347Q56 360 70 367H681L671 377Q638 412 609 458T580 514Z"
                  />
                </g>
              </symbol>
              <symbol id="rightarrow_3982089376" viewBox="0 -647.8 1000.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 237T56 250T70 270H835Q719 357 692 493Q692 494 692 496T691 499Q691 511 708 511H711Q720 511 723 510T729 506T732 497T735 481T743 456Q765 389 816 336T935 261Q944 258 944 250Q944 244 939 241T915 231T877 212Q836 186 806 152T761 85T740 35T732 4Q730 -6 727 -8T711 -11Q691 -11 691 0Q691 7 696 25Q728 151 835 230H70Q56 237 56 250Z"
                  />
                </g>
              </symbol>
              <symbol id="sigma_1312148424" viewBox="0 -576.1 572.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M184 -11Q116 -11 74 34T31 147Q31 247 104 333T274 430Q275 431 414 431H552Q553 430 555 429T559 427T562 425T565 422T567 420T569 416T570 412T571 407T572 401Q572 357 507 357Q500 357 490 357T476 358H416L421 348Q439 310 439 263Q439 153 359 71T184 -11ZM361 278Q361 358 276 358Q152 358 115 184Q114 180 114 178Q106 141 106 117Q106 67 131 47T188 26Q242 26 287 73Q316 103 334 153T356 233T361 278Z"
                  />
                </g>
              </symbol>
              <symbol id="sigma_1312149480" viewBox="0 -791.3 722.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M666 247Q664 244 652 126T638 4V0H351Q131 0 95 0T57 5V6Q54 12 57 17L73 36Q89 54 121 90T182 159L305 299L56 644L55 658Q55 677 60 681Q63 683 351 683H638V679Q640 674 652 564T666 447V443H626V447Q618 505 604 543T559 605Q529 626 478 631T333 637H294H189L293 494Q314 465 345 422Q400 346 400 340Q400 338 399 337L154 57Q407 57 428 58Q476 60 508 68T551 83T575 103Q595 125 608 162T624 225L626 251H666V247Z"
                  />
                </g>
              </symbol>
              <symbol id="sim_2087688366" viewBox="0 -504.3 778.5 578.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 166Q55 241 101 304T222 367Q260 367 296 349T362 304T421 252T484 208T554 189Q616 189 655 236T694 338Q694 350 698 358T708 367Q722 367 722 334Q722 260 677 197T562 134H554Q517 134 481 152T414 196T355 248T292 293T223 311Q179 311 145 286Q109 257 96 218T80 156T69 133Q55 133 55 166Z"
                  />
                </g>
              </symbol>
              <symbol id="simeq_1929181338" viewBox="0 -576.1 778.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 283Q55 356 103 409T217 463Q262 463 297 447T395 382Q431 355 446 344T493 320T554 307H558Q613 307 652 344T694 433Q694 464 708 464T722 432Q722 356 673 304T564 251H554Q510 251 465 275T387 329T310 382T223 407H219Q164 407 122 367Q91 333 85 295T76 253T69 250Q55 250 55 283ZM56 56Q56 71 72 76H706Q722 70 722 56Q722 44 707 36H70Q56 43 56 56Z"
                  />
                </g>
              </symbol>
              <symbol id="sqrtnx_3625618123" viewBox="0 -863.1 1406 1295.7">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <g transform="translate(208,336)">
                    <path
                      stroke-width="1"
                      transform="scale(0.574)"
                      d="M21 287Q22 293 24 303T36 341T56 388T89 425T135 442Q171 442 195 424T225 390T231 369Q231 367 232 367L243 378Q304 442 382 442Q436 442 469 415T503 336T465 179T427 52Q427 26 444 26Q450 26 453 27Q482 32 505 65T540 145Q542 153 560 153Q580 153 580 145Q580 144 576 130Q568 101 554 73T508 17T439 -10Q392 -10 371 17T350 73Q350 92 386 193T423 345Q423 404 379 404H374Q288 404 229 303L222 291L189 157Q156 26 151 16Q138 -11 108 -11Q95 -11 87 -5T76 7T74 17Q74 30 112 180T152 343Q153 348 153 366Q153 405 129 405Q91 405 66 305Q60 285 60 284Q58 278 41 278H27Q21 284 21 287Z"
                    />
                  </g>
                  <path
                    stroke-width="1"
                    d="M95 178Q89 178 81 186T72 200T103 230T169 280T207 309Q209 311 212 311H213Q219 311 227 294T281 177Q300 134 312 108L397 -77Q398 -77 501 136T707 565T814 786Q820 800 834 800Q841 800 846 794T853 782V776L620 293L385 -193Q381 -200 366 -200Q357 -200 354 -197Q352 -195 256 15L160 225L144 214Q129 202 113 190T95 178Z"
                    transform="translate(0,-109)"
                  />
                  <rect stroke="none" width="572" height="60" x="833" y="632" />
                  <g transform="translate(833,0)">
                    <path
                      stroke-width="1"
                      d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="sqrtx_1093998339" viewBox="0 -863.1 1406 1295.7">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M95 178Q89 178 81 186T72 200T103 230T169 280T207 309Q209 311 212 311H213Q219 311 227 294T281 177Q300 134 312 108L397 -77Q398 -77 501 136T707 565T814 786Q820 800 834 800Q841 800 846 794T853 782V776L620 293L385 -193Q381 -200 366 -200Q357 -200 354 -197Q352 -195 256 15L160 225L144 214Q129 202 113 190T95 178Z"
                    transform="translate(0,-109)"
                  />
                  <rect stroke="none" width="572" height="60" x="833" y="632" />
                  <g transform="translate(833,0)">
                    <path
                      stroke-width="1"
                      d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="star_194646573" viewBox="0 -576.1 500.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M210 282Q210 284 225 381T241 480Q241 484 245 484Q249 486 251 486Q258 486 260 477T272 406Q275 390 276 380Q290 286 290 282L388 299Q484 314 487 314H488Q497 314 497 302Q497 297 434 266Q416 257 404 251L315 206L361 118Q372 98 383 75T401 40L407 28Q407 16 395 16Q394 16 392 16L390 17L250 159L110 17L108 16Q106 16 105 16Q93 16 93 28L99 40Q105 52 116 75T139 118L185 206L96 251Q6 296 4 300Q3 301 3 302Q3 314 12 314H13Q16 314 112 299L210 282Z"
                  />
                </g>
              </symbol>
              <symbol id="subset_1805468287" viewBox="0 -647.8 778.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M84 250Q84 372 166 450T360 539Q361 539 370 539T395 539T430 540T475 540T524 540H679Q694 532 694 520Q694 511 681 501L522 500H470H441Q366 500 338 496T266 472Q244 461 224 446T179 404T139 337T124 250V245Q124 157 185 89Q244 25 328 7Q348 2 366 2T522 0H681Q694 -10 694 -20Q694 -32 679 -40H526Q510 -40 480 -40T434 -41Q350 -41 289 -25T172 45Q84 127 84 250Z"
                  />
                </g>
              </symbol>
              <symbol id="subseteq_2051895947" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M84 346Q84 468 166 546T360 635Q361 635 370 635T395 635T430 636T475 636T524 636H679Q694 628 694 616Q694 607 681 597L522 596H470H441Q366 596 338 592T266 568Q244 557 224 542T179 500T139 433T124 346V341Q124 253 185 185Q244 121 328 103Q348 98 366 98T522 96H681Q694 86 694 76Q694 64 679 56H526Q510 56 480 56T434 55Q350 55 289 71T172 141Q84 223 84 346ZM104 -131T104 -118T118 -98H679Q694 -106 694 -118T679 -138H118Q104 -131 104 -118Z"
                  />
                </g>
              </symbol>
              <symbol id="succ_176814527" viewBox="0 -647.8 778.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M84 517Q84 539 102 539Q115 539 119 529T125 503T137 459T171 404Q277 275 640 269H661Q694 269 694 249T661 229H640Q526 227 439 214T283 173T173 98T124 -17Q118 -41 103 -41Q83 -41 83 -17Q88 29 105 67T151 132T208 178T277 212T342 233T404 249Q401 250 380 254T345 263T302 276T245 299Q125 358 92 468Q84 502 84 517Z"
                  />
                </g>
              </symbol>
              <symbol id="succeq_3517341131" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M84 614Q84 636 102 636Q115 636 119 626T125 600T137 556T171 501Q277 372 640 366H661Q694 366 694 346T661 326H640Q578 325 526 321T415 307T309 280T222 237T156 172T124 83Q122 66 118 62T103 57Q100 57 98 57T95 58T93 59T90 62T85 67Q83 71 83 80Q88 126 105 164T151 229T208 275T277 309T342 330T404 346Q401 347 380 351T345 360T302 373T245 396Q125 455 92 565Q84 599 84 614ZM84 -131T84 -118T98 -98H679Q694 -106 694 -118T679 -138H98Q84 -131 84 -118Z"
                  />
                </g>
              </symbol>
              <symbol id="sum_abx_3782346798" viewBox="0 -1867.7 2183.7 3161.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M60 948Q63 950 665 950H1267L1325 815Q1384 677 1388 669H1348L1341 683Q1320 724 1285 761Q1235 809 1174 838T1033 881T882 898T699 902H574H543H251L259 891Q722 258 724 252Q725 250 724 246Q721 243 460 -56L196 -356Q196 -357 407 -357Q459 -357 548 -357T676 -358Q812 -358 896 -353T1063 -332T1204 -283T1307 -196Q1328 -170 1348 -124H1388Q1388 -125 1381 -145T1356 -210T1325 -294L1267 -449L666 -450Q64 -450 61 -448Q55 -446 55 -439Q55 -437 57 -433L590 177Q590 178 557 222T452 366T322 544L56 909L55 924Q55 945 60 948Z"
                  />
                  <g transform="translate(535,-1051)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                  <g transform="translate(570,1151)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                  <g transform="translate(1611,0)">
                    <path
                      stroke-width="1"
                      d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="supset_1806220461" viewBox="0 -647.8 778.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M83 520Q83 532 98 540H251Q267 540 297 540T343 541Q427 541 488 525T605 455Q693 374 693 250Q693 165 650 99T545 0T415 -39Q407 -40 251 -40H98Q83 -32 83 -20Q83 -10 96 0H255H308H337Q412 0 439 4T512 28Q533 39 553 54T599 96T639 163T654 250Q654 341 592 411Q557 449 512 472Q468 491 439 495T335 500H306H255L96 501Q83 511 83 520Z"
                  />
                </g>
              </symbol>
              <symbol id="supseteq_2051566041" viewBox="0 -719.6 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M83 616Q83 628 98 636H251Q267 636 297 636T343 637Q427 637 488 621T605 551Q693 470 693 346Q693 261 650 195T545 96T415 57Q407 56 251 56H98Q83 64 83 76Q83 86 96 96H255H308H337Q412 96 439 100T512 124Q533 135 553 150T599 192T639 259T654 346Q654 437 592 507Q557 545 512 568Q468 587 439 591T335 596H306H255L96 597Q83 607 83 616ZM84 -131T84 -118T98 -98H659Q674 -106 674 -118T659 -138H98Q84 -131 84 -118Z"
                  />
                </g>
              </symbol>
              <symbol id="tau_2087983641" viewBox="0 -576.1 517.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M39 284Q18 284 18 294Q18 301 45 338T99 398Q134 425 164 429Q170 431 332 431Q492 431 497 429Q517 424 517 402Q517 388 508 376T485 360Q479 358 389 358T299 356Q298 355 283 274T251 109T233 20Q228 5 215 -4T186 -13Q153 -13 153 20V30L203 192Q214 228 227 272T248 336L254 357Q254 358 208 358Q206 358 197 358T183 359Q105 359 61 295Q56 287 53 286T39 284Z"
                  />
                </g>
              </symbol>
              <symbol id="theta_1322769205" viewBox="0 -791.3 469.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M35 200Q35 302 74 415T180 610T319 704Q320 704 327 704T339 705Q393 701 423 656Q462 596 462 495Q462 380 417 261T302 66T168 -10H161Q125 -10 99 10T60 63T41 130T35 200ZM383 566Q383 668 330 668Q294 668 260 623T204 521T170 421T157 371Q206 370 254 370L351 371Q352 372 359 404T375 484T383 566ZM113 132Q113 26 166 26Q181 26 198 36T239 74T287 161T335 307L340 324H145Q145 321 136 286T120 208T113 132Z"
                  />
                </g>
              </symbol>
              <symbol id="theta_1322770389" viewBox="0 -791.3 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M56 340Q56 423 86 494T164 610T270 680T388 705Q521 705 621 601T722 341Q722 260 693 191T617 75T510 4T388 -22T267 3T160 74T85 189T56 340ZM610 339Q610 428 590 495T535 598T463 651T384 668Q332 668 289 638T221 566Q168 485 168 339Q168 274 176 235Q189 158 228 105T324 28Q356 16 388 16Q415 16 442 24T501 54T555 111T594 205T610 339ZM223 263V422H263V388H514V422H554V263H514V297H263V263H223Z"
                  />
                </g>
              </symbol>
              <symbol id="tildex_2388847703" viewBox="0 -791.3 572.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                  />
                  <g transform="translate(63,336)">
                    <path
                      stroke-width="1"
                      d="M179 251Q164 251 151 245T131 234T111 215L97 227L83 238Q83 239 95 253T121 283T142 304Q165 318 187 318T253 300T320 282Q335 282 348 288T368 299T388 318L402 306L416 295Q375 236 344 222Q330 215 313 215Q292 215 248 233T179 251Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="times_2159110463" viewBox="0 -576.1 778.5 649.8">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M630 29Q630 9 609 9Q604 9 587 25T493 118L389 222L284 117Q178 13 175 11Q171 9 168 9Q160 9 154 15T147 29Q147 36 161 51T255 146L359 250L255 354Q174 435 161 449T147 471Q147 480 153 485T168 490Q173 490 175 489Q178 487 284 383L389 278L493 382Q570 459 587 475T609 491Q630 491 630 471Q630 464 620 453T522 355L418 250L522 145Q606 61 618 48T630 29Z"
                  />
                </g>
              </symbol>
              <symbol id="top_2088097970" viewBox="0 -791.3 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 642T55 648T59 659T66 666T71 668H708Q723 660 723 648T708 628H409V15Q402 2 391 0Q387 0 384 1T379 3T375 6T373 9T371 13T369 16V628H71Q70 628 67 630T59 637Z"
                  />
                </g>
              </symbol>
              <symbol id="triangle_1315666263" viewBox="0 -791.3 889.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M75 0L72 2Q69 3 67 5T62 11T59 20Q59 24 62 30Q65 37 245 370T428 707Q428 708 430 710T436 714T444 716Q451 716 455 712Q459 710 644 368L828 27V20Q828 7 814 0H75ZM610 347L444 653Q443 653 278 347T113 40H775Q775 42 610 347Z"
                  />
                </g>
              </symbol>
              <symbol id="uparrow_3989604837" viewBox="0 -791.3 611.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M228 -179Q227 -180 226 -182T223 -186T221 -189T218 -192T214 -193T208 -194Q196 -194 189 -181L188 125V430L176 419Q122 369 59 338Q46 330 40 330Q38 330 31 337V350Q31 362 33 365T46 374Q60 381 77 390T128 426T190 484T247 567T292 677Q295 688 298 692Q302 694 305 694Q313 694 318 677Q334 619 363 568T420 485T481 427T532 391T564 374Q575 368 577 365T579 350V337Q572 330 570 330Q564 330 551 338Q487 370 435 419L423 430L422 125V-181Q409 -194 401 -194Q397 -194 394 -193T388 -189T385 -184T382 -180V-177V475L373 487Q331 541 305 602Q304 601 300 591T290 571T278 548T260 519T238 488L229 476L228 148V-179Z"
                  />
                </g>
              </symbol>
              <symbol id="uparrow_3989605765" viewBox="0 -791.3 500.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M27 414Q17 414 17 433Q17 437 17 439T17 444T19 447T20 450T22 452T26 453T30 454T36 456Q80 467 120 494T180 549Q227 607 238 678Q240 694 251 694Q259 694 261 684Q261 677 265 659T284 608T320 549Q340 525 363 507T405 479T440 463T467 455T479 451Q483 447 483 433Q483 413 472 413Q467 413 458 416Q342 448 277 545L270 555V-179Q262 -193 252 -193H250H248Q236 -193 230 -179V555L223 545Q192 499 146 467T70 424T27 414Z"
                  />
                </g>
              </symbol>
              <symbol id="updownarrow_3181819383" viewBox="0 -863.1 500.5 1223.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M27 492Q17 492 17 511Q17 515 17 517T17 522T19 525T20 528T22 530T26 531T30 532T36 534Q80 545 120 572T180 627Q210 664 223 701T238 755T250 772T261 762Q261 757 264 741T282 691T319 628Q352 589 390 566T454 536L479 529Q483 525 483 511Q483 491 472 491Q467 491 458 494Q342 526 277 623L270 633V-133L277 -123Q307 -77 353 -45T430 -2T473 8Q483 8 483 -11Q483 -15 483 -17T483 -22T481 -25T480 -28T478 -30T474 -31T470 -32T464 -34Q407 -49 364 -84T300 -157T270 -223T261 -262Q259 -272 250 -272Q242 -272 239 -255T223 -201T180 -127Q169 -112 155 -99T125 -76T95 -58T67 -45T44 -36T27 -31L21 -29Q17 -25 17 -11Q17 9 28 9Q33 9 42 6Q158 -26 223 -123L230 -133V633L223 623Q192 577 146 545T70 502T27 492Z"
                  />
                </g>
              </symbol>
              <symbol id="updownarrow_3181820311" viewBox="0 -863.1 611.5 1223.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M290 755Q298 772 305 772T318 757T343 706T393 633Q431 588 473 558T545 515T579 497V484Q579 464 570 464Q564 464 550 470Q485 497 423 550L422 400V100L423 -50Q485 3 550 30Q565 36 570 36Q579 36 579 16V3Q575 -1 549 -12T480 -53T393 -132Q361 -172 342 -208T318 -258T305 -272T293 -258T268 -208T217 -132Q170 -80 128 -51T61 -12T31 3V16Q31 36 40 36Q46 36 61 30Q86 19 109 6T146 -18T173 -38T188 -50V550Q186 549 173 539T147 519T110 495T61 470Q46 464 40 464Q31 464 31 484V497Q34 500 63 513T135 557T217 633Q267 692 290 755ZM374 598Q363 610 351 625T332 651T316 676T305 695L294 676Q282 657 267 636T236 598L228 589V-89L236 -98Q247 -110 259 -125T278 -151T294 -176T305 -195L316 -176Q328 -157 343 -136T374 -98L382 -89V589L374 598Z"
                  />
                </g>
              </symbol>
              <symbol id="upsilon_3526401323" viewBox="0 -576.1 540.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M413 384Q413 406 432 424T473 443Q492 443 507 425T523 367Q523 334 508 270T468 153Q424 63 373 27T282 -10H268Q220 -10 186 2T135 36T111 78T104 121Q104 170 138 262T173 379Q173 380 173 381Q173 390 173 393T169 400T158 404H154Q131 404 112 385T82 344T65 302T57 280Q55 278 41 278H27Q21 284 21 287Q21 299 34 333T82 404T161 441Q200 441 225 419T250 355Q248 336 247 334Q247 331 232 291T201 199T185 118Q185 68 211 47T275 26Q317 26 355 57T416 132T452 216T465 277Q465 301 457 318T439 343T421 361T413 384Z"
                  />
                </g>
              </symbol>
              <symbol id="upsilon_3526402507" viewBox="0 -791.3 778.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 551Q55 604 91 654T194 705Q240 705 277 681T334 624T367 556T385 498L389 474L392 488Q394 501 400 521T414 566T438 615T473 659T521 692T584 705Q620 705 648 689T691 647T714 597T722 551Q722 540 719 538T699 536Q680 536 677 541Q677 542 677 544T676 548Q676 576 650 596T588 616H582Q538 616 505 582Q466 543 454 477T441 318Q441 301 441 269T442 222V61Q448 55 452 53T478 48T542 46H569V0H557Q533 3 389 3T221 0H209V46H236Q256 46 270 46T295 47T311 48T322 51T328 54T332 57T337 61V209Q337 383 333 415Q313 616 189 616Q154 616 128 597T101 548Q101 540 97 538T78 536Q63 536 59 538T55 551Z"
                  />
                </g>
              </symbol>
              <symbol id="varepsilon_960150590" viewBox="0 -576.1 466.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M190 -22Q124 -22 76 11T27 107Q27 174 97 232L107 239L99 248Q76 273 76 304Q76 364 144 408T290 452H302Q360 452 405 421Q428 405 428 392Q428 381 417 369T391 356Q382 356 371 365T338 383T283 392Q217 392 167 368T116 308Q116 289 133 272Q142 263 145 262T157 264Q188 278 238 278H243Q308 278 308 247Q308 206 223 206Q177 206 142 219L132 212Q68 169 68 112Q68 39 201 39Q253 39 286 49T328 72T345 94T362 105Q376 103 376 88Q376 79 365 62T334 26T275 -8T190 -22Z"
                  />
                </g>
              </symbol>
              <symbol id="varphi_1675249005" viewBox="0 -576.1 654.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M92 210Q92 176 106 149T142 108T185 85T220 72L235 70L237 71L250 112Q268 170 283 211T322 299T370 375T429 423T502 442Q547 442 582 410T618 302Q618 224 575 152T457 35T299 -10Q273 -10 273 -12L266 -48Q260 -83 252 -125T241 -179Q236 -203 215 -212Q204 -218 190 -218Q159 -215 159 -185Q159 -175 214 -2L209 0Q204 2 195 5T173 14T147 28T120 46T94 71T71 103T56 142T50 190Q50 238 76 311T149 431H162Q183 431 183 423Q183 417 175 409Q134 361 114 300T92 210ZM574 278Q574 320 550 344T486 369Q437 369 394 329T323 218Q309 184 295 109L286 64Q304 62 306 62Q423 62 498 131T574 278Z"
                  />
                </g>
              </symbol>
              <symbol id="varpi_1640113541" viewBox="0 -576.1 828.5 721.6">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M206 -10Q158 -10 136 24T114 110Q114 233 199 349L205 358H184Q144 358 121 347Q108 340 95 330T75 312T61 295T53 285Q51 284 38 284Q19 284 19 294Q19 300 38 329T93 391T164 429Q171 431 532 431Q799 431 803 430Q823 423 823 402Q823 377 801 364Q790 358 766 358Q748 358 748 357Q748 355 749 348T752 327T754 297Q754 258 738 207T693 107T618 24T520 -10Q488 -10 466 2T432 36T416 77T411 120Q411 128 410 128T404 122Q373 71 323 31T206 -10ZM714 296Q714 316 707 358H251Q250 357 244 348T230 328T212 301T193 267T176 229T164 187T159 144Q159 62 222 62Q290 62 349 127T432 285Q433 286 434 288T435 291T437 293T440 294T444 294T452 294H466Q472 288 472 286Q472 285 464 244T456 170Q456 62 534 62Q604 62 659 139T714 296Z"
                  />
                </g>
              </symbol>
              <symbol id="varrho_4099271785" viewBox="0 -576.1 517.5 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M205 -174Q136 -174 102 -153T67 -76Q67 -25 91 85T127 234Q143 289 182 341Q252 427 341 441Q343 441 349 441T359 442Q432 442 471 394T510 276Q510 169 431 80T253 -10Q226 -10 204 -2T169 19T146 44T132 64L128 73Q128 72 124 53T116 5T112 -44Q112 -68 117 -78T150 -95T236 -102Q327 -102 356 -111T386 -154Q386 -166 384 -178Q381 -190 378 -192T361 -194H348Q342 -188 342 -179Q342 -169 315 -169Q294 -169 264 -171T205 -174ZM424 322Q424 359 407 382T357 405Q322 405 287 376T231 300Q221 276 204 217Q188 152 188 116Q188 68 210 47T259 26Q297 26 334 62Q367 92 389 158T418 266T424 322Z"
                  />
                </g>
              </symbol>
              <symbol id="varsigma_233085869" viewBox="0 -576.1 405.5 793.3">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M31 207Q31 306 115 374T302 442Q341 442 373 430T405 400Q405 392 399 383T379 374Q373 375 348 390T296 405Q222 405 160 357T98 249Q98 232 103 218T112 195T132 175T154 159T186 141T219 122Q234 114 255 102T286 85T299 78L302 74Q306 71 308 69T315 61T322 51T328 40T332 25T334 8Q334 -31 305 -69T224 -107Q194 -107 163 -92Q156 -88 156 -80Q156 -73 162 -67T178 -61Q186 -61 190 -63Q209 -71 224 -71Q244 -71 253 -59T263 -30Q263 -25 263 -21T260 -12T255 -4T248 3T239 9T227 17T213 25T195 34T174 46Q170 48 150 58T122 74T97 90T70 112T51 137T36 169T31 207Z"
                  />
                </g>
              </symbol>
              <symbol id="vartheta_3952813808" viewBox="0 -791.3 591.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M537 500Q537 474 533 439T524 383L521 362Q558 355 561 351Q563 349 563 345Q563 321 552 318Q542 318 521 323L510 326Q496 261 459 187T362 51T241 -11Q100 -11 100 105Q100 139 127 242T154 366Q154 405 128 405Q107 405 92 377T68 316T57 280Q55 278 41 278H27Q21 284 21 287Q21 291 27 313T47 368T79 418Q103 442 134 442Q169 442 201 419T233 344Q232 330 206 228T180 98Q180 26 247 26Q292 26 332 90T404 260L427 349Q422 349 398 359T339 392T289 440Q265 476 265 520Q265 590 312 647T417 705Q463 705 491 670T528 592T537 500ZM464 564Q464 668 413 668Q373 668 339 622T304 522Q304 494 317 470T349 431T388 406T421 391T435 387H436L443 415Q450 443 457 485T464 564Z"
                  />
                </g>
              </symbol>
              <symbol id="vee_2087413967" viewBox="0 -719.6 667.5 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M55 580Q56 587 61 592T75 598Q86 598 96 580L333 48L570 580Q579 596 586 597Q588 598 591 598Q609 598 611 580Q611 574 546 426T415 132T348 -15Q343 -22 333 -22T318 -15Q317 -14 252 131T121 425T55 580Z"
                  />
                </g>
              </symbol>
              <symbol id="wedge_1162335725" viewBox="0 -719.6 667.5 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M318 591Q325 598 333 598Q344 598 348 591Q349 590 414 445T545 151T611 -4Q609 -22 591 -22Q588 -22 586 -21T581 -20T577 -17T575 -13T572 -9T570 -4L333 528L96 -4Q87 -20 80 -21Q78 -22 75 -22Q57 -22 55 -4Q55 2 120 150T251 444T318 591Z"
                  />
                </g>
              </symbol>
              <symbol id="x_a_193458981" viewBox="0 -576.1 1046.9 865.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                  />
                  <g transform="translate(572,-150)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="x_ab_3953731551" viewBox="0 -934.9 1046.9 1223.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                  />
                  <g transform="translate(572,352)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                  <g transform="translate(572,-149)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="xb_193427367" viewBox="0 -1006.6 976.2 1152.1">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M52 289Q59 331 106 386T222 442Q257 442 286 424T329 379Q371 442 430 442Q467 442 494 420T522 361Q522 332 508 314T481 292T458 288Q439 288 427 299T415 328Q415 374 465 391Q454 404 425 404Q412 404 406 402Q368 386 350 336Q290 115 290 78Q290 50 306 38T341 26Q378 26 414 59T463 140Q466 150 469 151T485 153H489Q504 153 504 145Q504 144 502 134Q486 77 440 33T333 -11Q263 -11 227 52Q186 -10 133 -10H127Q78 -10 57 16T35 71Q35 103 54 123T99 143Q142 143 142 101Q142 81 130 66T107 46T94 41L91 40Q91 39 97 36T113 29T132 26Q168 26 194 71Q203 87 217 139T245 247T261 313Q266 340 266 352Q266 380 251 392T217 404Q177 404 142 372T93 290Q91 281 88 280T72 278H58Q52 284 52 289Z"
                  />
                  <g transform="translate(572,412)">
                    <path
                      stroke-width="1"
                      transform="scale(0.707)"
                      d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
                    />
                  </g>
                </g>
              </symbol>
              <symbol id="xi_193417672" viewBox="0 -791.3 667.5 936.9">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M47 509L55 676Q55 677 333 677T611 676L619 509Q619 508 599 508T579 510Q579 529 575 557T564 589Q550 594 333 594T102 589Q95 586 91 558T87 510Q87 508 67 508T47 509ZM139 260V445H179V394H487V445H527V260H487V311H179V260H139ZM50 0L42 180H62Q82 180 82 178Q82 133 89 105Q92 93 95 90T108 86Q137 83 333 83Q530 83 558 86Q568 87 571 90T577 105Q584 133 584 178Q584 180 604 180H624L616 0H50Z"
                  />
                </g>
              </symbol>
              <symbol id="xi_193418728" viewBox="0 -791.3 443.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M268 632Q268 704 296 704Q314 704 314 687Q314 682 311 664T308 635T309 620V616H315Q342 619 360 619Q443 619 443 586Q439 548 358 546H344Q326 546 317 549T290 566Q257 550 226 505T195 405Q195 381 201 364T211 342T218 337Q266 347 298 347Q375 347 375 314Q374 297 359 288T327 277T280 275Q234 275 208 283L195 286Q149 260 119 214T88 130Q88 116 90 108Q101 79 129 63T229 20Q238 17 243 15Q337 -21 354 -33Q383 -53 383 -94Q383 -137 351 -171T273 -205Q240 -205 202 -190T158 -167Q156 -163 156 -159Q156 -151 161 -146T176 -140Q182 -140 189 -143Q232 -168 274 -168Q286 -168 292 -165Q313 -151 313 -129Q313 -112 301 -104T232 -75Q214 -68 204 -64Q198 -62 171 -52T136 -38T107 -24T78 -8T56 12T36 37T26 66T21 103Q21 149 55 206T145 301L154 307L148 313Q141 319 136 323T124 338T111 358T103 382T99 413Q99 471 143 524T259 602L271 607Q268 618 268 632Z"
                  />
                </g>
              </symbol>

              <symbol id="zeta_170233907" viewBox="0 -791.3 471.5 1080.4">
                <g
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  transform="matrix(1 0 0 -1 0 0)"
                >
                  <path
                    stroke-width="1"
                    d="M296 643Q298 704 324 704Q342 704 342 687Q342 682 339 664T336 633Q336 623 337 618T338 611Q339 612 341 612Q343 614 354 616T374 618L384 619H394Q471 619 471 586Q467 548 386 546H372Q338 546 320 564L311 558Q235 506 175 398T114 190Q114 171 116 155T125 127T137 104T153 86T171 72T192 61T213 53T235 46T256 39L322 16Q389 -10 389 -80Q389 -119 364 -154T300 -202Q292 -204 274 -204Q247 -204 225 -196Q210 -192 193 -182T172 -167Q167 -159 173 -148Q180 -139 191 -139Q195 -139 221 -153T283 -168Q298 -166 310 -152T322 -117Q322 -91 302 -75T250 -51T183 -29T116 4T65 62T44 160Q44 287 121 410T293 590L302 595Q296 613 296 643Z"
                  />
                </g>
              </symbol>
            </svg>
          </span>
        </div>

        <!-- add by dream  矩阵-->
        <svg id="MJX-SVG-global-cache1" style="display: none;">
          <defs>
            <path
              id="MJX-TEX-S3-28"
              d="M701 -940Q701 -943 695 -949H664Q662 -947 636 -922T591 -879T537 -818T475 -737T412 -636T350 -511T295 -362T250 -186T221 17T209 251Q209 962 573 1361Q596 1386 616 1405T649 1437T664 1450H695Q701 1444 701 1441Q701 1436 681 1415T629 1356T557 1261T476 1118T400 927T340 675T308 359Q306 321 306 250Q306 -139 400 -430T690 -924Q701 -936 701 -940Z"
            />
            <path
              id="MJX-TEX-I-61"
              d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
            />
            <path
              id="MJX-TEX-I-62"
              d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
            />
            <path
              id="MJX-TEX-I-63"
              d="M34 159Q34 268 120 355T306 442Q362 442 394 418T427 355Q427 326 408 306T360 285Q341 285 330 295T319 325T330 359T352 380T366 386H367Q367 388 361 392T340 400T306 404Q276 404 249 390Q228 381 206 359Q162 315 142 235T121 119Q121 73 147 50Q169 26 205 26H209Q321 26 394 111Q403 121 406 121Q410 121 419 112T429 98T420 83T391 55T346 25T282 0T202 -11Q127 -11 81 37T34 159Z"
            />
            <path
              id="MJX-TEX-I-64"
              d="M366 683Q367 683 438 688T511 694Q523 694 523 686Q523 679 450 384T375 83T374 68Q374 26 402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487H491Q506 153 506 145Q506 140 503 129Q490 79 473 48T445 8T417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157Q33 205 53 255T101 341Q148 398 195 420T280 442Q336 442 364 400Q369 394 369 396Q370 400 396 505T424 616Q424 629 417 632T378 637H357Q351 643 351 645T353 664Q358 683 366 683ZM352 326Q329 405 277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q233 26 290 98L298 109L352 326Z"
            />
            <path
              id="MJX-TEX-S3-29"
              d="M34 1438Q34 1446 37 1448T50 1450H56H71Q73 1448 99 1423T144 1380T198 1319T260 1238T323 1137T385 1013T440 864T485 688T514 485T526 251Q526 134 519 53Q472 -519 162 -860Q139 -885 119 -904T86 -936T71 -949H56Q43 -949 39 -947T34 -937Q88 -883 140 -813Q428 -430 428 251Q428 453 402 628T338 922T245 1146T145 1309T46 1425Q44 1427 42 1429T39 1433T36 1436L34 1438Z"
            />
          </defs>
        </svg>
        <svg id="MJX-SVG-global-cache2" style="display: none;">
          <defs>
            <path id="MJX-TEX-S3-5B" d="M247 -949V1450H516V1388H309V-887H516V-949H247Z" />
            <path
              id="MJX-TEX-I-61"
              d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
            />
            <path
              id="MJX-TEX-I-62"
              d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
            />
            <path
              id="MJX-TEX-I-63"
              d="M34 159Q34 268 120 355T306 442Q362 442 394 418T427 355Q427 326 408 306T360 285Q341 285 330 295T319 325T330 359T352 380T366 386H367Q367 388 361 392T340 400T306 404Q276 404 249 390Q228 381 206 359Q162 315 142 235T121 119Q121 73 147 50Q169 26 205 26H209Q321 26 394 111Q403 121 406 121Q410 121 419 112T429 98T420 83T391 55T346 25T282 0T202 -11Q127 -11 81 37T34 159Z"
            />
            <path
              id="MJX-TEX-I-64"
              d="M366 683Q367 683 438 688T511 694Q523 694 523 686Q523 679 450 384T375 83T374 68Q374 26 402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487H491Q506 153 506 145Q506 140 503 129Q490 79 473 48T445 8T417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157Q33 205 53 255T101 341Q148 398 195 420T280 442Q336 442 364 400Q369 394 369 396Q370 400 396 505T424 616Q424 629 417 632T378 637H357Q351 643 351 645T353 664Q358 683 366 683ZM352 326Q329 405 277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q233 26 290 98L298 109L352 326Z"
            />
            <path id="MJX-TEX-S3-5D" d="M11 1388V1450H280V-949H11V-887H218V1388H11Z" />
          </defs>
        </svg>
        <svg id="MJX-SVG-global-cache3" style="display: none;">
          <defs>
            <path
              id="MJX-TEX-S3-7B"
              d="M618 -943L612 -949H582L568 -943Q472 -903 411 -841T332 -703Q327 -682 327 -653T325 -350Q324 -28 323 -18Q317 24 301 61T264 124T221 171T179 205T147 225T132 234Q130 238 130 250Q130 255 130 258T131 264T132 267T134 269T139 272T144 275Q207 308 256 367Q310 436 323 519Q324 529 325 851Q326 1124 326 1154T332 1205Q369 1358 566 1443L582 1450H612L618 1444V1429Q618 1413 616 1411L608 1406Q599 1402 585 1393T552 1372T515 1343T479 1305T449 1257T429 1200Q425 1180 425 1152T423 851Q422 579 422 549T416 498Q407 459 388 424T346 364T297 318T250 284T214 264T197 254L188 251L205 242Q290 200 345 138T416 3Q421 -18 421 -48T423 -349Q423 -397 423 -472Q424 -677 428 -694Q429 -697 429 -699Q434 -722 443 -743T465 -782T491 -816T519 -845T548 -868T574 -886T595 -899T610 -908L616 -910Q618 -912 618 -928V-943Z"
            />
            <path
              id="MJX-TEX-I-61"
              d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
            />
            <path
              id="MJX-TEX-I-62"
              d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
            />
            <path
              id="MJX-TEX-I-63"
              d="M34 159Q34 268 120 355T306 442Q362 442 394 418T427 355Q427 326 408 306T360 285Q341 285 330 295T319 325T330 359T352 380T366 386H367Q367 388 361 392T340 400T306 404Q276 404 249 390Q228 381 206 359Q162 315 142 235T121 119Q121 73 147 50Q169 26 205 26H209Q321 26 394 111Q403 121 406 121Q410 121 419 112T429 98T420 83T391 55T346 25T282 0T202 -11Q127 -11 81 37T34 159Z"
            />
            <path
              id="MJX-TEX-I-64"
              d="M366 683Q367 683 438 688T511 694Q523 694 523 686Q523 679 450 384T375 83T374 68Q374 26 402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487H491Q506 153 506 145Q506 140 503 129Q490 79 473 48T445 8T417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157Q33 205 53 255T101 341Q148 398 195 420T280 442Q336 442 364 400Q369 394 369 396Q370 400 396 505T424 616Q424 629 417 632T378 637H357Q351 643 351 645T353 664Q358 683 366 683ZM352 326Q329 405 277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q233 26 290 98L298 109L352 326Z"
            />
            <path
              id="MJX-TEX-S3-7D"
              d="M131 1414T131 1429T133 1447T148 1450H153H167L182 1444Q276 1404 336 1343T415 1207Q421 1184 421 1154T423 851L424 531L426 517Q434 462 460 415T518 339T571 296T608 274Q615 270 616 267T618 251Q618 241 618 238T615 232T608 227Q542 194 491 132T426 -15L424 -29L423 -350Q422 -622 422 -652T415 -706Q397 -780 337 -841T182 -943L167 -949H153Q137 -949 134 -946T131 -928Q131 -914 132 -911T144 -904Q146 -903 148 -902Q299 -820 323 -680Q324 -663 325 -349T327 -19Q355 145 541 241L561 250L541 260Q356 355 327 520Q326 537 325 850T323 1181Q315 1227 293 1267T244 1332T193 1374T151 1401T132 1413Q131 1414 131 1429Z"
            />
          </defs>
        </svg>
        <svg id="MJX-SVG-global-cache4" style="display: none;">
          <defs>
            <path
              id="MJX-TEX-S4-2223"
              d="M139 -249H137Q125 -249 119 -235V251L120 737Q130 750 139 750Q152 750 159 735V-235Q151 -249 141 -249H139Z"
            />
            <path
              id="MJX-TEX-I-61"
              d="M33 157Q33 258 109 349T280 441Q331 441 370 392Q386 422 416 422Q429 422 439 414T449 394Q449 381 412 234T374 68Q374 43 381 35T402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487Q506 153 506 144Q506 138 501 117T481 63T449 13Q436 0 417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157ZM351 328Q351 334 346 350T323 385T277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q217 26 254 59T298 110Q300 114 325 217T351 328Z"
            />
            <path
              id="MJX-TEX-I-62"
              d="M73 647Q73 657 77 670T89 683Q90 683 161 688T234 694Q246 694 246 685T212 542Q204 508 195 472T180 418L176 399Q176 396 182 402Q231 442 283 442Q345 442 383 396T422 280Q422 169 343 79T173 -11Q123 -11 82 27T40 150V159Q40 180 48 217T97 414Q147 611 147 623T109 637Q104 637 101 637H96Q86 637 83 637T76 640T73 647ZM336 325V331Q336 405 275 405Q258 405 240 397T207 376T181 352T163 330L157 322L136 236Q114 150 114 114Q114 66 138 42Q154 26 178 26Q211 26 245 58Q270 81 285 114T318 219Q336 291 336 325Z"
            />
            <path
              id="MJX-TEX-I-63"
              d="M34 159Q34 268 120 355T306 442Q362 442 394 418T427 355Q427 326 408 306T360 285Q341 285 330 295T319 325T330 359T352 380T366 386H367Q367 388 361 392T340 400T306 404Q276 404 249 390Q228 381 206 359Q162 315 142 235T121 119Q121 73 147 50Q169 26 205 26H209Q321 26 394 111Q403 121 406 121Q410 121 419 112T429 98T420 83T391 55T346 25T282 0T202 -11Q127 -11 81 37T34 159Z"
            />
            <path
              id="MJX-TEX-I-64"
              d="M366 683Q367 683 438 688T511 694Q523 694 523 686Q523 679 450 384T375 83T374 68Q374 26 402 26Q411 27 422 35Q443 55 463 131Q469 151 473 152Q475 153 483 153H487H491Q506 153 506 145Q506 140 503 129Q490 79 473 48T445 8T417 -8Q409 -10 393 -10Q359 -10 336 5T306 36L300 51Q299 52 296 50Q294 48 292 46Q233 -10 172 -10Q117 -10 75 30T33 157Q33 205 53 255T101 341Q148 398 195 420T280 442Q336 442 364 400Q369 394 369 396Q370 400 396 505T424 616Q424 629 417 632T378 637H357Q351 643 351 645T353 664Q358 683 366 683ZM352 326Q329 405 277 405Q242 405 210 374T160 293Q131 214 119 129Q119 126 119 118T118 106Q118 61 136 44T179 26Q233 26 290 98L298 109L352 326Z"
            />
          </defs>
        </svg>

        <!-- add by drea-->
        <div id="thehelp">
          <!-- 希腊字母 -->
          <div>
            <div>
              <div>
                <div
                  class="Popover-content Popover-content--bottom Popover-content--arrowed"
                  id="Popover7-content"
                  aria-labelledby="Popover7-toggle"
                  style="left: 344.4px; top: 80.2px;"
                >
                  <span
                    class="Popover-arrow Popover-arrow--bottom"
                    style="left: 204.8px; top: 0px;"
                  ></span>
                  <div class="MathToolbar-palettes MathToolbar-palettes--greek">
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.488ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#alpha_1318469421" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.332ex"
                        height="2.509ex"
                        style="vertical-align: -0.671ex;"
                      >
                        <title />
                        <use xlink:href="#beta_170234155" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.262ex"
                        height="2.176ex"
                        style="vertical-align: -0.838ex;"
                      >
                        <title />
                        <use xlink:href="#gamma_1311936542" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.049ex"
                        height="2.343ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#delta_1323077121" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="0.944ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#epsilon_3526400827" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.083ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#varepsilon_960150590" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.095ex"
                        height="2.509ex"
                        style="vertical-align: -0.671ex;"
                      >
                        <title />
                        <use xlink:href="#zeta_170233907" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.169ex"
                        height="2.176ex"
                        style="vertical-align: -0.838ex;"
                      >
                        <title />
                        <use xlink:href="#eta_2087567017" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.09ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#theta_1322769205" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.374ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#vartheta_3952813808" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="0.823ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#iota_170249034" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.339ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#kappa_1326819122" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.355ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#lambda_79015454" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.402ex"
                        height="2.176ex"
                        style="vertical-align: -0.838ex;"
                      >
                        <title />
                        <use xlink:href="#mu_193423105" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.232ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#nu_193423138" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.03ex"
                        height="2.509ex"
                        style="vertical-align: -0.671ex;"
                      >
                        <title />
                        <use xlink:href="#xi_193418728" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.332ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#pi_193418976" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.924ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#varpi_1640113541" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.202ex"
                        height="2.176ex"
                        style="vertical-align: -0.838ex;"
                      >
                        <title />
                        <use xlink:href="#rho_2087900556" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.202ex"
                        height="2.009ex"
                        style="vertical-align: -0.671ex;"
                      >
                        <title />
                        <use xlink:href="#varrho_4099271785" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.33ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#sigma_1312148424" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="0.942ex"
                        height="1.843ex"
                        style="vertical-align: -0.505ex;"
                      >
                        <title />
                        <use xlink:href="#varsigma_233085869" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.202ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#tau_2087983641" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.255ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#upsilon_3526401323" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.385ex"
                        height="2.509ex"
                        style="vertical-align: -0.671ex;"
                      >
                        <title />
                        <use xlink:href="#phi_2087833096" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.52ex"
                        height="2.176ex"
                        style="vertical-align: -0.838ex;"
                      >
                        <title />
                        <use xlink:href="#varphi_1675249005" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.455ex"
                        height="2.009ex"
                        style="vertical-align: -0.671ex;"
                      >
                        <title />
                        <use xlink:href="#chi_2087832827" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.513ex"
                        height="2.509ex"
                        style="vertical-align: -0.671ex;"
                      >
                        <title />
                        <use xlink:href="#psi_2087863571" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.446ex"
                        height="1.676ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#omega_1309716888" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.453ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#gamma_1311937598" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.936ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#delta_1323076193" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#theta_1322770389" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.613ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#lambda_79016510" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.55ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#xi_193417672" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.743ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#pi_193417920" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.678ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#sigma_1312149480" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#upsilon_3526402507" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.678ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#phi_2087834152" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#psi_2087862643" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.678ex"
                        height="2.176ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#omega_1309717944" />
                      </svg>
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <!-- 交集并集 -->
          <div>
            <div
              class="Popover-content Popover-content--top Popover-content--arrowed Popover-content-enter-done"
              id="Popover7-content"
              aria-labelledby="Popover7-toggle"
              style="left: 409px; top: 202.2px;"
            >
              <span class="Popover-arrow Popover-arrow--top" style="left: 205px; top: 122px;"></span>
              <div class="MathToolbar-palettes MathToolbar-palettes--symbol">
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="1.509ex"
                    style="vertical-align: 0.019ex; margin-bottom: -0.19ex;"
                  >
                    <title />
                    <use xlink:href="#times_2159110463" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="1.843ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#div_2088172514" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="0.647ex"
                    height="1.176ex"
                    style="vertical-align: 0.439ex; margin-bottom: -0.61ex;"
                  >
                    <title />
                    <use xlink:href="#cdot_183307621" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#pm_193414628" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.009ex"
                    style="vertical-align: -0.671ex;"
                  >
                    <title />
                    <use xlink:href="#mp_193426244" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.162ex"
                    height="1.509ex"
                    style="vertical-align: 0.079ex; margin-bottom: -0.25ex;"
                  >
                    <title />
                    <use xlink:href="#ast_2087933023" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.162ex"
                    height="1.509ex"
                    style="vertical-align: 0.035ex; margin-bottom: -0.206ex;"
                  >
                    <title />
                    <use xlink:href="#star_194646573" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.162ex"
                    height="1.509ex"
                    style="vertical-align: 0.125ex; margin-bottom: -0.297ex;"
                  >
                    <title />
                    <use xlink:href="#circ_177147074" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.162ex"
                    height="1.509ex"
                    style="vertical-align: 0.125ex; margin-bottom: -0.297ex;"
                  >
                    <title />
                    <use xlink:href="#bullet_1823366303" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.176ex"
                    style="vertical-align: -0.505ex;"
                  >
                    <title />
                    <use xlink:href="#oplus_2140408172" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.176ex"
                    style="vertical-align: -0.505ex;"
                  >
                    <title />
                    <use xlink:href="#ominus_1906208922" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.176ex"
                    style="vertical-align: -0.505ex;"
                  >
                    <title />
                    <use xlink:href="#oslash_1351473011" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.176ex"
                    style="vertical-align: -0.505ex;"
                  >
                    <title />
                    <use xlink:href="#otimes_2531166928" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.176ex"
                    style="vertical-align: -0.505ex;"
                  >
                    <title />
                    <use xlink:href="#odot_183307497" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.032ex"
                    height="2.676ex"
                    style="vertical-align: -0.838ex;"
                  >
                    <title />
                    <use xlink:href="#dagger_1389219435" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.032ex"
                    height="2.509ex"
                    style="vertical-align: -0.671ex;"
                  >
                    <title />
                    <use xlink:href="#ddagger_2894567663" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.55ex"
                    height="2.009ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#vee_2087413967" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.55ex"
                    height="2.009ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#wedge_1162335725" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.55ex"
                    height="2.009ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#cap_2088096139" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.55ex"
                    height="2.009ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#cup_2088074399" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.42ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#aleph_1603530409" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.924ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#re_193404814" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.288ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#im_193412861" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#top_2088097970" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.808ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#bot_2087963360" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.324ex"
                    height="1.676ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#infty_2224287093" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.318ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#partial_1646186250" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.293ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#forall_4023651683" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.293ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#exists_1947215609" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.55ex"
                    height="1.176ex"
                    style="vertical-align: 0.204ex; margin-bottom: -0.376ex;"
                  >
                    <title />
                    <use xlink:href="#neg_2087624725" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.678ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#angle_1166132696" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.066ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#triangle_1315666263" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.162ex"
                    height="1.509ex"
                    style="vertical-align: 0.026ex; margin-bottom: -0.197ex;"
                  >
                    <title />
                    <use xlink:href="#diamond_2948158269" />
                  </svg>
                </button>
              </div>
            </div>
          </div>
          <!-- 大于小于 -->
          <div>
            <div>
              <div>
                <div
                  class="Popover-content Popover-content--top Popover-content--arrowed undefined Popover-content-enter-done"
                  id="Popover8-content"
                  aria-labelledby="Popover8-toggle"
                  style="left: 465px; top: 233.4px;"
                >
                  <span class="Popover-arrow Popover-arrow--top" style="left: 205px; top: 90px;"></span>
                  <div class="MathToolbar-palettes MathToolbar-palettes--comparison">
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.505ex;"
                      >
                        <title />
                        <use xlink:href="#leq_2088123777" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.505ex;"
                      >
                        <title />
                        <use xlink:href="#geq_2088123562" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.843ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#prec_176608541" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.843ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#succ_176814527" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.505ex;"
                      >
                        <title />
                        <use xlink:href="#preceq_3517413801" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.505ex;"
                      >
                        <title />
                        <use xlink:href="#succeq_3517341131" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="2.324ex"
                        height="1.843ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#ll_193412985" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="2.324ex"
                        height="1.843ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#gg_193411929" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.509ex"
                        style="vertical-align: 0.081ex; margin-bottom: -0.253ex;"
                      >
                        <title />
                        <use xlink:href="#equiv_1981568039" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.343ex"
                        style="vertical-align: 0.307ex; margin-bottom: -0.478ex;"
                      >
                        <title />
                        <use xlink:href="#sim_2087688366" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.509ex"
                        style="vertical-align: 0.081ex; margin-bottom: -0.253ex;"
                      >
                        <title />
                        <use xlink:href="#simeq_1929181338" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.509ex"
                        style="vertical-align: 0.035ex; margin-bottom: -0.206ex;"
                      >
                        <title />
                        <use xlink:href="#asymp_1903012079" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.509ex"
                        style="vertical-align: 0.125ex; margin-bottom: -0.297ex;"
                      >
                        <title />
                        <use xlink:href="#approx_3715476797" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.676ex"
                        style="vertical-align: -0.838ex;"
                      >
                        <title />
                        <use xlink:href="#ne_193405746" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.843ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#subset_1805468287" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="1.843ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#supset_1806220461" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.505ex;"
                      >
                        <title />
                        <use xlink:href="#subseteq_2051895947" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.808ex"
                        height="2.176ex"
                        style="vertical-align: -0.505ex;"
                      >
                        <title />
                        <use xlink:href="#supseteq_2051566041" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.55ex"
                        height="1.843ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#in_193419230" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.55ex"
                        height="1.843ex"
                        style="vertical-align: -0.338ex;"
                      >
                        <title />
                        <use xlink:href="#ni_193418046" />
                      </svg>
                    </button>
                    <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                      <svg
                        class="MathToolbar-paletteIcon"
                        width="1.55ex"
                        height="2.676ex"
                        style="vertical-align: -0.838ex;"
                      >
                        <title />
                        <use xlink:href="#notin_1650680875" />
                      </svg>
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <!-- 微积分 -->
          <div>
            <div
              class="Popover-content Popover-content--bottom Popover-content--arrowed Popover-content-enter-done"
              id="Popover9-content"
              aria-labelledby="Popover9-toggle"
              style="left: 523px; top: 350.6px;"
            >
              <span class="Popover-arrow Popover-arrow--bottom" style="left: 203px; top: 0px;"></span>
              <div class="MathToolbar-palettes MathToolbar-palettes--math">
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.432ex"
                    height="2.009ex"
                    style="vertical-align: -0.671ex;"
                  >
                    <title />
                    <use xlink:href="#x_a_193458981" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.267ex"
                    height="2.676ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#xb_193427367" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.432ex"
                    height="2.843ex"
                    style="vertical-align: -0.671ex;"
                  >
                    <title />
                    <use xlink:href="#x_ab_3953731551" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.33ex"
                    height="2.009ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#barx_3677001718" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.33ex"
                    height="2.176ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#tildex_2388847703" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.066ex"
                    height="4.843ex"
                    style="vertical-align: -2.005ex;"
                  >
                    <title />
                    <use xlink:href="#fracab_2392580300" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="3.266ex"
                    height="3.009ex"
                    style="vertical-align: -1.005ex;"
                  >
                    <title />
                    <use xlink:href="#sqrtx_1093998339" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="3.266ex"
                    height="3.009ex"
                    style="vertical-align: -1.005ex;"
                  >
                    <title />
                    <use xlink:href="#sqrtnx_3625618123" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.582ex"
                    height="7.343ex"
                    style="vertical-align: -3.005ex;"
                  >
                    <title />
                    <use xlink:href="#bigcap_ab_1527652773" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.582ex"
                    height="7.343ex"
                    style="vertical-align: -3.005ex;"
                  >
                    <title />
                    <use xlink:href="#bigcup_ab_745232433" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.969ex"
                    height="7.343ex"
                    style="vertical-align: -3.005ex;"
                  >
                    <title />
                    <use xlink:href="#prod_ab_1740866194" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.969ex"
                    height="7.343ex"
                    style="vertical-align: -3.005ex;"
                  >
                    <title />
                    <use xlink:href="#coprod_ab_1722723006" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="3.139ex"
                    height="2.843ex"
                    style="vertical-align: -0.838ex;"
                  >
                    <title />
                    <use xlink:href="#leftxright_308942087" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.623ex"
                    height="2.843ex"
                    style="vertical-align: -0.838ex;"
                  >
                    <title />
                    <use xlink:href="#leftxright_1060968992" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="3.655ex"
                    height="2.843ex"
                    style="vertical-align: -0.838ex;"
                  >
                    <title />
                    <use xlink:href="#leftxright_1282332352" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.623ex"
                    height="2.843ex"
                    style="vertical-align: -0.838ex;"
                  >
                    <title />
                    <use xlink:href="#leftxright_791668582" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="3.402ex"
                    height="6.343ex"
                    style="vertical-align: -2.338ex;"
                  >
                    <title />
                    <use xlink:href="#int_ab_3437240392" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="3.402ex"
                    height="6.343ex"
                    style="vertical-align: -2.338ex;"
                  >
                    <title />
                    <use xlink:href="#oint_ab_1759784839" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="5.072ex"
                    height="7.343ex"
                    style="vertical-align: -3.005ex;"
                  >
                    <title />
                    <use xlink:href="#sum_abx_3782346798" />
                  </svg>
                </button>

                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="4.946ex"
                    height="3.843ex"
                    style="vertical-align: -2.005ex;"
                  >
                    <title />
                    <use xlink:href="#lim_arightarrowbx_3542561616" />
                  </svg>
                </button>

                <!-- 矩阵 -->
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="7.966ex"
                    height="5.428ex"
                    role="img"
                    focusable="false"
                    viewBox="0 -1449.5 3521 2399"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    style="vertical-align: -2.148ex;"
                  >
                    <g
                      stroke="currentColor"
                      fill="currentColor"
                      stroke-width="0"
                      transform="matrix(1 0 0 -1 0 0)"
                    >
                      <g data-mml-node="math">
                        <g data-mml-node="mrow">
                          <g data-mml-node="mo">
                            <use
                              xmlns:xlink="http://www.w3.org/1999/xlink"
                              xlink:href="#MJX-TEX-S3-28"
                            />
                          </g>
                          <g data-mml-node="mtable" transform="translate(736, 0)">
                            <g data-mml-node="mtr" transform="translate(0, 636.5)">
                              <g data-mml-node="mtd">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-61"
                                  />
                                </g>
                              </g>
                              <g data-mml-node="mtd" transform="translate(1574.5, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-62"
                                  />
                                </g>
                              </g>
                            </g>
                            <g data-mml-node="mtr" transform="translate(0, -636.5)">
                              <g data-mml-node="mtd" transform="translate(48, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-63"
                                  />
                                </g>
                              </g>
                              <g data-mml-node="mtd" transform="translate(1529, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-64"
                                  />
                                </g>
                              </g>
                            </g>
                          </g>
                          <g data-mml-node="mo" transform="translate(2785, 0)">
                            <use
                              xmlns:xlink="http://www.w3.org/1999/xlink"
                              xlink:href="#MJX-TEX-S3-29"
                            />
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </button>

                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="7.025ex"
                    height="5.428ex"
                    role="img"
                    focusable="false"
                    viewBox="0 -1449.5 3105 2399"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    style="vertical-align: -2.148ex;"
                  >
                    <g
                      stroke="currentColor"
                      fill="currentColor"
                      stroke-width="0"
                      transform="matrix(1 0 0 -1 0 0)"
                    >
                      <g data-mml-node="math">
                        <g data-mml-node="mrow">
                          <g data-mml-node="mo">
                            <use
                              xmlns:xlink="http://www.w3.org/1999/xlink"
                              xlink:href="#MJX-TEX-S3-5B"
                            />
                          </g>
                          <g data-mml-node="mtable" transform="translate(528, 0)">
                            <g data-mml-node="mtr" transform="translate(0, 636.5)">
                              <g data-mml-node="mtd">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-61"
                                  />
                                </g>
                              </g>
                              <g data-mml-node="mtd" transform="translate(1574.5, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-62"
                                  />
                                </g>
                              </g>
                            </g>
                            <g data-mml-node="mtr" transform="translate(0, -636.5)">
                              <g data-mml-node="mtd" transform="translate(48, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-63"
                                  />
                                </g>
                              </g>
                              <g data-mml-node="mtd" transform="translate(1529, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-64"
                                  />
                                </g>
                              </g>
                            </g>
                          </g>
                          <g data-mml-node="mo" transform="translate(2577, 0)">
                            <use
                              xmlns:xlink="http://www.w3.org/1999/xlink"
                              xlink:href="#MJX-TEX-S3-5D"
                            />
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </button>

                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="8.029ex"
                    height="5.428ex"
                    role="img"
                    focusable="false"
                    viewBox="0 -1449.5 3549 2399"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    style="vertical-align: -2.148ex;"
                  >
                    <g
                      stroke="currentColor"
                      fill="currentColor"
                      stroke-width="0"
                      transform="matrix(1 0 0 -1 0 0)"
                    >
                      <g data-mml-node="math">
                        <g data-mml-node="mrow">
                          <g data-mml-node="mo">
                            <use
                              xmlns:xlink="http://www.w3.org/1999/xlink"
                              xlink:href="#MJX-TEX-S3-7B"
                            />
                          </g>
                          <g data-mml-node="mtable" transform="translate(750, 0)">
                            <g data-mml-node="mtr" transform="translate(0, 636.5)">
                              <g data-mml-node="mtd">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-61"
                                  />
                                </g>
                              </g>
                              <g data-mml-node="mtd" transform="translate(1574.5, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-62"
                                  />
                                </g>
                              </g>
                            </g>
                            <g data-mml-node="mtr" transform="translate(0, -636.5)">
                              <g data-mml-node="mtd" transform="translate(48, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-63"
                                  />
                                </g>
                              </g>
                              <g data-mml-node="mtd" transform="translate(1529, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-64"
                                  />
                                </g>
                              </g>
                            </g>
                          </g>
                          <g data-mml-node="mo" transform="translate(2799, 0)">
                            <use
                              xmlns:xlink="http://www.w3.org/1999/xlink"
                              xlink:href="#MJX-TEX-S3-7D"
                            />
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </button>

                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="5.894ex"
                    height="5.143ex"
                    role="img"
                    focusable="false"
                    viewBox="0 -1386.5 2605 2273.1"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    style="vertical-align: -2.006ex;"
                  >
                    <g
                      stroke="currentColor"
                      fill="currentColor"
                      stroke-width="0"
                      transform="matrix(1 0 0 -1 0 0)"
                    >
                      <g data-mml-node="math">
                        <g data-mml-node="mrow">
                          <g data-mml-node="mo">
                            <svg
                              width="278"
                              height="2273.1"
                              y="-886.5"
                              x="0"
                              viewBox="0 -281.6 278 2273.1"
                            >
                              <use
                                xmlns:xlink="http://www.w3.org/1999/xlink"
                                xlink:href="#MJX-TEX-S4-2223"
                                transform="scale(1, 3.413)"
                              />
                            </svg>
                          </g>
                          <g data-mml-node="mtable" transform="translate(278, 0)">
                            <g data-mml-node="mtr" transform="translate(0, 636.5)">
                              <g data-mml-node="mtd">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-61"
                                  />
                                </g>
                              </g>
                              <g data-mml-node="mtd" transform="translate(1574.5, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-62"
                                  />
                                </g>
                              </g>
                            </g>
                            <g data-mml-node="mtr" transform="translate(0, -636.5)">
                              <g data-mml-node="mtd" transform="translate(48, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-63"
                                  />
                                </g>
                              </g>
                              <g data-mml-node="mtd" transform="translate(1529, 0)">
                                <g data-mml-node="mi">
                                  <use
                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                    xlink:href="#MJX-TEX-I-64"
                                  />
                                </g>
                              </g>
                            </g>
                          </g>
                          <g data-mml-node="mo" transform="translate(2327, 0)">
                            <svg
                              width="278"
                              height="2273.1"
                              y="-886.5"
                              x="0"
                              viewBox="0 -281.6 278 2273.1"
                            >
                              <use
                                xmlns:xlink="http://www.w3.org/1999/xlink"
                                xlink:href="#MJX-TEX-S4-2223"
                                transform="scale(1, 3.413)"
                              />
                            </svg>
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </button>

                <!-- /矩阵 -->
              </div>
            </div>
          </div>

          <!-- 箭头 -->
          <div>
            <div
              class="Popover-content Popover-content--top Popover-content--arrowed Popover-content-enter-done"
              id="Popover10-content"
              aria-labelledby="Popover10-toggle"
              style="left: 577px; top: 253.6px;"
            >
              <span class="Popover-arrow Popover-arrow--top" style="left: 205px; top: 69px;"></span>
              <div class="MathToolbar-palettes MathToolbar-palettes--arrow">
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.324ex"
                    height="1.843ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#leftarrow_2463726907" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.324ex"
                    height="1.843ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#rightarrow_3982089376" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.324ex"
                    height="1.843ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#leftrightarrow_2294107771" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.324ex"
                    height="1.843ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#leftarrow_2463728091" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.324ex"
                    height="1.843ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#rightarrow_3982088320" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="2.324ex"
                    height="1.843ex"
                    style="vertical-align: -0.338ex;"
                  >
                    <title />
                    <use xlink:href="#leftrightarrow_2294108699" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.162ex"
                    height="2.509ex"
                    style="vertical-align: -0.671ex;"
                  >
                    <title />
                    <use xlink:href="#uparrow_3989605765" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.162ex"
                    height="2.509ex"
                    style="vertical-align: -0.671ex;"
                  >
                    <title />
                    <use xlink:href="#downarrow_2479725650" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.162ex"
                    height="2.843ex"
                    style="vertical-align: -0.838ex;"
                  >
                    <title />
                    <use xlink:href="#updownarrow_3181819383" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.42ex"
                    height="2.509ex"
                    style="vertical-align: -0.671ex;"
                  >
                    <title />
                    <use xlink:href="#uparrow_3989604837" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.42ex"
                    height="2.509ex"
                    style="vertical-align: -0.671ex;"
                  >
                    <title />
                    <use xlink:href="#downarrow_2479726706" />
                  </svg>
                </button>
                <button type="button" class="Button MathToolbar-palettesButton Button--plain">
                  <svg
                    class="MathToolbar-paletteIcon"
                    width="1.42ex"
                    height="2.843ex"
                    style="vertical-align: -0.838ex;"
                  >
                    <title />
                    <use xlink:href="#updownarrow_3181820311" />
                  </svg>
                </button>
              </div>
            </div>
          </div>
        </div>公式：
        <input
          style="border:solid 1px #ccc; padding:10px; width:500px; "
          type="text"
          id="result"
          name="result"
        />

        <input
          type="button"
          value="确定"
          @click="setForum(); return false;"
          style="border:solid 1px #ccc; padding:8px; width:60px;"
        />

        <input type="checkbox" id="isbr" name="isbr" />换行
      </form>
  </div>
  </div>
</template>

<script>
export default {
  // name: "Mathsymbols",
  props: {
    visible: {
      type: Boolean,
      required: false
    },
    noSave: {
      type: Boolean,
      required: false
    },
  },
  data() {
    return {
      symbols: [
        "\\alpha",
        "\\beta",
        "\\gamma",
        "\\delta",
        "\\epsilon",
        "\\varepsilon",
        "\\zeta",
        "\\eta",
        "\\theta",
        "\\vartheta",
        "\\iota",
        "\\kappa",
        "\\lambda",
        "\\mu",
        "\\nu",
        "\\xi",
        "\\pi",
        "\\varpi",
        "\\rho",
        "\\varrho",
        "\\sigma",
        "\\varsigma",
        "\\tau",
        "\\upsilon",
        "\\phi",
        "\\varphi",
        "\\chi",
        "\\psi",
        "\\omega",
        "\\Gamma",
        "\\Delta",
        "\\Theta",
        "\\Lambda",
        "\\Xi",
        "\\Pi",
        "\\Sigma",
        "\\Upsilon",
        "\\Phi",
        "\\Psi",
        "\\Omega",

        "\\times",
        "\\div",
        "\\cdot",
        "\\pm",
        "\\mp",
        "\\ast",
        "\\star",
        "\\circ",
        "\\bullet",
        "\\oplus",
        "\\ominus",
        "\\oslash",
        "\\otimes",
        "\\odot",
        "\\dagger",
        "\\ddagger",
        "\\vee",
        "\\wedge",
        "\\cap",
        "\\cup",
        "\\aleph",
        "\\Re",
        "\\Im",
        "\\top",
        "\\bot",
        "\\infty",
        "\\partial",
        "\\forall",
        "\\exists",
        "\\neg",
        "\\angle",
        "\\triangle",
        "\\diamond",

        "\\leq",
        "\\geq",
        "\\prec",
        "\\succ",
        "\\preceq",
        "\\succeq",
        "\\ll",
        "\\gg",
        "\\equiv",
        "\\sim",
        "\\simeq",
        "\\asymp",
        "\\approx",
        "\\ne",
        "\\subset",
        "\\supset",
        "\\subseteq",
        "\\supseteq",
        "\\in",
        "\\ni",
        "\\notin",

        "x_{a}",
        "x^{b}",
        "x_{a}^{b}",
        "\\bar{x}",
        "\\tilde{x}",
        "\\frac{a}{b}",
        "\\sqrt{x}",
        "\\sqrt[n]{x}",
        "\\bigcap_{a}^{b}",
        "\\bigcup_{a}^{b}",
        "\\prod_{a}^{b}",
        "\\coprod_{a}^{b}",
        "\\left( x \\right)",
        "\\left[ x \\right]",
        "\\left\\{ x \\right\\}",
        "\\left| x \\right|",
        "\\int_{a}^{b}",
        "\\oint_{a}^{b}",
        "\\sum_{a}^{b}{x}",
        "\\lim_{a \\rightarrow b}{x}",
        "\\begin{pmatrix}a&b\\\\c&d\\\\ \\end{pmatrix}",
        "\\begin{bmatrix}a&b\\\\c&d\\\\ \\end{bmatrix}",
        "\\begin{Bmatrix}a&b\\\\c&d\\\\ \\end{Bmatrix}",
        "\\begin{vmatrix}a&b\\\\c&d\\\\ \\end{vmatrix}",

        "\\leftarrow",
        "\\rightarrow",
        "\\leftrightarrow",
        "\\Leftarrow",
        "\\Rightarrow",
        "\\Leftrightarrow",
        "\\uparrow",
        "\\downarrow",
        "\\updownarrow",
        "\\Uparrow",
        "\\Downarrow",
        "\\Updownarrow"
      ],
    };
  },

  mounted() {
    let than = this;

    window.$("#thehelp button").each(function(idx) {

      window.$(this).click(function() {
        if (idx >= 102 && idx <= 117) {
          window.$("#isbr").prop("checked", true);
        } else {
          window.$("#isbr").prop("checked", false);
        }
        console.log(window.$("#result").val());
        console.log(than.symbols[idx]);

        window.$("#result").val(window.$("#result").val() + " " + than.symbols[idx]);
      });
    });
  },
  methods: {
    setForum() {
      var _id = '<%=Request.QueryString["id"] %>';
      var _isbr = window.$("#isbr").is(":checked");
      var html = "";
    console.log(html);
    
      if (_isbr) {
        html =
          `<p><span class="math-tex">\\[ ` +
          window.$("#result").val() +
          ` \\]</span></p>`;
      } else {
        html =
          `<p><span class="math-tex">\\( ` +
          window.$("#result").val() +
          ` \\)</span></p>`;
      }
      parent.tinyMCE.execCommand("mceReplaceContent", false, html, _id);
      // parent.layer.closeAll();
       this.close()
      window.$("#result").val("")

    },
     //关闭子页面
    close: function() {
      this.$emit('update:visible', false)
    },
     //触发父组件处理事件
  /*   saveBut: function() {
      this.close()
      this.$emit('save-but')
    },

    handleClose(){

    } */
  }
};
</script>

<style lang="less" scoped>
.MathToolbar-paletteIcon {
  font-size: 13px;
}
.MathToolbar-palettesButton {
  margin: 4px;
  padding: 0;
  height: 24px;
  -webkit-box-pack: center;
  -ms-flex-pack: center;
  justify-content: center;
  -webkit-box-align: center;
  -ms-flex-align: center;
  align-items: center;
  font-size: 16px;
  color: #8590a6;
  border-radius: 4px;
}

.MathToolbar-palettes,
.MathToolbar-palettesButton {
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
  -webkit-box-orient: horizontal;
  -webkit-box-direction: normal;
  -ms-flex-direction: row;
  flex-direction: row;
}

.Button--link,
.Button--plain {
  height: auto;
  padding: 0;
  line-height: inherit;
  background-color: transparent;
  border: none;
  border-radius: 0;
}

.Button {
  display: inline-block;
  padding: 0 16px;
  font-size: 14px;
  line-height: 32px;
  color: #8590a6;
  text-align: center;
  cursor: pointer;
  background: none;
  border: 1px solid;
  border-radius: 3px;
}

.MathToolbar-palettes {
  -ms-flex-wrap: wrap;
  flex-wrap: wrap;
  margin: -4px;
  padding: 16px;
  max-width: 100%;
}
/* 制作成弹框的样式 */
.Mathematical {
  border: 1px solid #ddd;
  background: #fff;
    position: absolute;
  z-index: 4;
  top: 200px;
  left: 50%;
  // margin: auto;
  margin-left: -600px;
  // margin-top:- calc((100vh - 84px)/ 2);
  width: 1200px;
  // height: calc(100vh - 84px);
  min-height: 75%;
  // padding-top: 84px;
  background: #fff;
  transition: all 0.5s;
  transform: scale(0);
  .sybol {
    position: relative;

    .title {
      // line-height: 20px;
      padding: 10px;
      background: #ccc;
      width: 100%;
      display: flex;
      justify-content: space-between;
          height: 47px;
    box-sizing: border-box;
    }
    .close {
      font-size: 30px;
    cursor: pointer;
    }
  }
}
.animation-scale {
  transform: scale(1);
}
</style>